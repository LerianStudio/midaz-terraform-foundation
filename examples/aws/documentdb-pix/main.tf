...
resource "aws_docdb_cluster_parameter_group" "pix" {
  family      = "docdb5.0"
  name        = "${local.name}-params"
  description = "DocumentDB cluster parameter group for PIX"
...
resource "aws_docdb_cluster" "main" {
  ...
  db_cluster_parameter_group_name = aws_docdb_cluster_parameter_group.pix.name
