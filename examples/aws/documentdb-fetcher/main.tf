...
resource "aws_docdb_cluster_parameter_group" "fetcher" {
  family      = "docdb5.0"
  name        = "${local.name}-params"
  description = "DocumentDB cluster parameter group for Fetcher"
...
resource "aws_docdb_cluster" "main" {
  ...
  db_cluster_parameter_group_name = aws_docdb_cluster_parameter_group.fetcher.name
