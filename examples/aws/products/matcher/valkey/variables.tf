...
variable "product" {
  description = "Product this replication group belongs to. Pinned to \"matcher\" by validation: the derived name (matcher-{env}-valkey) and the derived secret path (matcher-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the matcher Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "matcher"

  validation {
    condition     = var.product == "matcher"
    error_message = "The product must be \"matcher\". This root stack is the matcher valkey datastore; every name and secret path it produces is derived from it. To provision valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# reason: the matcher workloads run on the EKS nodes, and the replication group has to be
...
variable "port" {
  description = "Port the cache nodes accept connections on. Whether the application wants it as a separate variable or folded into the host string is UNKNOWN — the Matcher chart is not in this repository, and the readable Lerian charts do it both ways. The port is published as its own output; the release wiring decides the shape."
  type        = number
  default     = 6379
}
...
variable "transit_encryption_mode" {
  description = "Transit encryption mode: \"preferred\" accepts both TLS and plaintext clients, \"required\" accepts TLS only. Kept at preferred because there is no chart to confirm the application is TLS-capable, and requiring TLS from an unknown client is how a deployment fails at connect time rather than at plan time."
  type        = string
  default     = "preferred"
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Defaults to false because there is no chart to confirm the application sends AUTH. Turn it on once the Matcher team confirms it does — the token is already waiting in the secret."
  type        = bool
  default     = false
}
