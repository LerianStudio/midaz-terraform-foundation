...
variable "product" {
  description = "Product this broker belongs to. Pinned to \"matcher\" by validation: the derived name (matcher-{env}-rabbitmq[-single|-cluster]) and the derived secret path (matcher-{env}-rabbitmq/password) ARE the cross-stack discovery contract. A different value here silently produces resources the matcher Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "matcher"

  validation {
    condition     = var.product == "matcher"
    error_message = "The product must be \"matcher\". This root stack is the matcher rabbitmq datastore; every name and secret path it produces is derived from it. To provision rabbitmq for another product, copy this directory to examples/aws/products/<product>/rabbitmq instead."
  }
}
...
variable "shared_broker_name" {
  description = <<-EOT
    ...
    matcher release consuming the shared broker there sets:
...
# reason: the matcher workloads run on the EKS nodes, and the broker has to be
...
variable "port" {
  description = "AMQPS port the broker listens on. AmazonMQ for RabbitMQ exposes NO plaintext AMQP listener, so this is 5671 and there is no reason to change it. Which env var the Matcher application reads it from is unknown — the chart is not in this repository — so it is published through the port and amqp_endpoint outputs rather than through helm_values."
  type        = number
  default     = 5671
}

variable "console_port" {
  description = "Port of the RabbitMQ management console and management HTTP API. AmazonMQ serves both over HTTPS on 443. Whether the application health-checks the management API (midaz does, plugin-br-bank-transfer does not) is unknown, so the port is opened by default and the console_url output published."
  type        = number
  default     = 443
}

variable "enable_console_ingress" {
  description = "Also open console_port (443) to the resolved ingress sources, so the management console and the management HTTP API stay reachable from inside the VPC. Left on because a broker health check against the management API is a common Lerian pattern and there is no chart here to rule it out. The exposure is bounded by the ingress allow list, not by the port list."
  type        = bool
  default     = true
}
...
variable "mq_admin_user" {
  description = "Administrator username of the broker. Deliberately NOT marked sensitive at this level: the module marks its own copy sensitive, and reading it back through that output would redact anything it is merged into. In mode = \"shared\" this is the value the CALLER declares, not a read of the shared broker — keep it equal to the admin user of the shared tier (both default to \"rabbitmqadmin\")."
  type        = string
