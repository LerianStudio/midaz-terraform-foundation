################################################################################
# products/matcher/postgres — the PostgreSQL datastore of the matcher product
#
...
# state file (aws/products/matcher/postgres/terraform.tfstate). Its siblings —
...
# takes down one datastore instead of three.
#
#   mode = "dedicated"  -> creates matcher-{env}-postgres, its security group,
...
# module seeded with product = "matcher" would produce matcher-{env}-vpc and
# matcher-{env}-eks, which do not exist.
################################################################################
...
################################################################################
# PostgreSQL — matcher-{environment}-postgres
# Secret: matcher-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
