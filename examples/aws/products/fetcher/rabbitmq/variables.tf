...
variable "product" {
  description = "Product this broker belongs to. Pinned to \"fetcher\" by validation: the derived name (fetcher-{env}-rabbitmq[-single|-cluster]) and the derived secret path (fetcher-{env}-rabbitmq/password) ARE the cross-stack discovery contract. A different value here silently produces resources the fetcher Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "fetcher"

  validation {
    condition     = var.product == "fetcher"
    error_message = "The product must be \"fetcher\". This root stack is the fetcher broker; every name and secret path it produces is derived from it. To provision this service for another product, copy this directory to examples/aws/products/<product>/rabbitmq instead."
  }
}
...
variable "shared_broker_name" {
  description = <<-EOT
    ...
    fetcher release consuming the shared broker there sets:
...
# fetcher workloads run on the EKS nodes, and the broker has to be
...
variable "enable_console_ingress" {
  description = "Also open console_port (443) to the resolved ingress sources, so the management console and the management HTTP API stay reachable from inside the VPC. The fetcher chart's RABBITMQ_HEALTH_CHECK_URL targets exactly that API, and templates/bootstrap-rabbitmq.yaml drives it to declare the work queue and the job-events exchange — turning this off breaks both."
  type        = bool
  default     = true
}
