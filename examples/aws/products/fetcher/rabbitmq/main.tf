################################################################################
# products/fetcher/rabbitmq — the message broker of fetcher
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/fetcher/rabbitmq/terraform.tfstate). That matters most
...
#
#   mode = "dedicated"  -> creates fetcher-{env}-rabbitmq[-single|-cluster], its
...
################################################################################
# RabbitMQ — fetcher-{environment}-rabbitmq[-single|-cluster]
# Secret: fetcher-{env}-rabbitmq/password   Host: the raw AmazonMQ broker host
################################################################################
