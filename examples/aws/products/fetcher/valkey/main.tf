################################################################################
# products/fetcher/valkey — the cache datastore of fetcher
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/fetcher/valkey/terraform.tfstate). Its siblings are
# independent roots with independent state.
#
#   mode = "dedicated"  -> creates fetcher-{env}-valkey, its security group and
...
################################################################################
# Valkey — fetcher-{environment}-valkey
# Secret: fetcher-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
