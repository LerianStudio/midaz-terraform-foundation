...
variable "product" {
  description = "Product this cluster belongs to. Pinned to \"fetcher\" by validation: the derived name (fetcher-{env}-docdb) and the derived secret path (fetcher-{env}-docdb/password) ARE the cross-stack discovery contract. A different value here silently produces resources the fetcher Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "fetcher"

  validation {
    condition     = var.product == "fetcher"
    error_message = "The product must be \"fetcher\". This root stack is the fetcher cluster; every name and secret path it produces is derived from it. To provision this service for another product, copy this directory to examples/aws/products/<product>/documentdb instead."
  }
}
...
# fetcher workloads run on the EKS nodes, and the cluster has to be
