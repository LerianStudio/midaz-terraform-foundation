################################################################################
# products/reporter/documentdb — the MongoDB-compatible datastore of reporter
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/reporter/documentdb/terraform.tfstate). Its siblings are
...
# of four.
#
#   mode = "dedicated"  -> creates reporter-{env}-docdb, its security group, its
...
################################################################################
# DocumentDB — reporter-{environment}-docdb
# Secret: reporter-{env}-docdb/password   Host: the raw docdb writer endpoint
################################################################################
