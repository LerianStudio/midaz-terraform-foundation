################################################################################
# products/reporter/s3 — the object storage of reporter
#
# ONE ROOT STACK PER SERVICE, same as the datastore siblings: one directory, one
# state file (aws/products/reporter/s3/terraform.tfstate).
#
...
################################################################################
# Object storage — reporter-{environment}-reporter-storage-{account_id}
#
# The account id suffix is the documented naming exception: S3 bucket names are
# globally unique across every AWS account, so reporter-{env}-reporter-storage alone
...
#
# reporter-storage holds the generated regulatory reports. It is the only
