...
variable "product" {
  description = "Product this replication group belongs to. Pinned to \"reporter\" by validation: the derived name (reporter-{env}-valkey) and the derived secret path (reporter-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the reporter Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "reporter"

  validation {
    condition     = var.product == "reporter"
    error_message = "The product must be \"reporter\". This root stack is the reporter replication group; every name and secret path it produces is derived from it. To provision this service for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# reporter workloads run on the EKS nodes, and the replication group has to be
...
variable "transit_encryption_mode" {
  description = "Transit encryption mode: \"preferred\" accepts both TLS and plaintext clients, \"required\" accepts TLS only. Keep it preferred while the reporter-helm chart still connects without TLS — flipping it to \"required\" without a chart change locks the application out."
  type        = string
  default     = "preferred"
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Defaults to false because the reporter-helm chart ships no Valkey AUTH client configuration by default — turning it on without wiring the password locks every consumer out."
  type        = bool
  default     = false
}
