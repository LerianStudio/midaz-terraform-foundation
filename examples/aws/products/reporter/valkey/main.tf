################################################################################
# products/reporter/valkey — the cache datastore of reporter
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/reporter/valkey/terraform.tfstate). Its siblings are
# independent roots with independent state.
#
#   mode = "dedicated"  -> creates reporter-{env}-valkey, its security group and
...
################################################################################
# Valkey — reporter-{environment}-valkey
# Secret: reporter-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
