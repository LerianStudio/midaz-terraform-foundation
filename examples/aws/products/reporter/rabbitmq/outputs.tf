...
output "secret_arn" {
  description = "ARN of the Secrets Manager secret holding the admin password: reporter-{env}-rabbitmq/password in dedicated mode, shared-{env}-rabbitmq/password in shared mode."
...
# `common.configmap` (the RABBITMQ block), rendered by
# templates/manager/configmap.yaml and templates/worker/configmap.yaml.
#
# ┌──────────────────────────────────────────────────────────────────────────┐
# │ THE PORT VARIABLES ARE THE OPPOSITE WAY ROUND FROM MIDAZ. Copying the     │
# │ midaz mapping here produces a manager that health-checks the AMQPS port   │
# │ and a worker that opens an AMQP connection to the management API.        │
# │                                                                          │
...
# └──────────────────────────────────────────────────────────────────────────┘
#
# The reporter direction is proven three times over in the chart, not inferred:
#   templates/NOTES.txt            "AMQP Port: {{ ...RABBITMQ_PORT_AMQP }}" and
#                                  "Management Port: {{ ...RABBITMQ_PORT_HOST }}"
#   templates/worker/keda-scaled-job.yaml
#                                  host: "{{ RABBITMQ_URI }}://{{ RABBITMQ_HOST
#                                  }}:{{ RABBITMQ_PORT_AMQP }}"  (an AMQP dial)
#   templates/manager/deployment.yaml
#                                  init container waits on
#                                  "$RABBITMQ_HOST:$RABBITMQ_PORT_HOST"
#                                  while worker/deployment.yaml waits on
#                                  "$RABBITMQ_HOST:$RABBITMQ_PORT_AMQP"
# and values.yaml ships PORT_AMQP: 5672 / PORT_HOST: 15672 to match.
#
...
# cannot connect at all, and the KEDA trigger above would build an amqp:// URL
# that never opens.
#
# RABBITMQ_HEALTH_CHECK_URL is emitted with NO port. AmazonMQ serves the
# management API over HTTPS on 443, and "https://host:443" is the same endpoint
# written the long way — but the chart also feeds this string to the bootstrap
# Job, so the shorter form is the one that matches the chart's own https branch.
#
# NOT emitted here, on purpose:
#   RABBITMQ_DEFAULT_PASS  — read from secret_name by External Secrets.
#   RABBITMQ_ERLANG_COOKIE — required only when the BUNDLED broker runs. With
#     rabbitmq.enabled = false there is no broker to give a cookie to; leave it
#     empty rather than inventing one.
#   RABBITMQ_EXCHANGE / RABBITMQ_GENERATE_REPORT_QUEUE / _KEY /
#     RABBITMQ_NUMBERS_OF_WORKERS — application topology and tuning. AmazonMQ
#     creates no exchanges or queues; the chart's bootstrap Job does, over the
#     management API.
#   RABBITMQ_PROTOCOL — does NOT exist in this chart's app configmap. It appears
#     only under global.externalRabbitmqDefinitions.connection.protocol, which
#     configures the bootstrap Job, not the application. Set that to "https" and
#     its port to 443 when pointing the Job at AmazonMQ.
#
# DESTINATION NOTE: every key below belongs in `common.configmap` EXCEPT
# RABBITMQ_DEFAULT_USER, which this chart keeps under `secrets:`. Merging it into
# the configmap instead leaves the Secret key empty and the broker credentials
# half-wired.
################################################################################

output "helm_values" {
  description = "reporter chart env vars this broker fills in. All keys merge into common.configmap except RABBITMQ_DEFAULT_USER, which the chart keeps under secrets. Pair it with rabbitmq.enabled = false so the bundled subchart is not deployed alongside AmazonMQ — the reporter chart ships rabbitmq.enabled = true by DEFAULT."
  value = {
    RABBITMQ_URI  = "amqps"
    ...
    # Belongs under `secrets:`, not `configmap:`.
