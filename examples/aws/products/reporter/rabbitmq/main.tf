################################################################################
# products/reporter/rabbitmq — the message broker of reporter
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/reporter/rabbitmq/terraform.tfstate). That matters most
...
#
#   mode = "dedicated"  -> creates reporter-{env}-rabbitmq[-single|-cluster], its
...
################################################################################
# RabbitMQ — reporter-{environment}-rabbitmq[-single|-cluster]
# Secret: reporter-{env}-rabbitmq/password   Host: the raw AmazonMQ broker host
################################################################################
