...
# the OPTIONAL multi-tenant connection-pool registry of tracer
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/tracer/valkey/terraform.tfstate). Its sibling —
...
#
#   mode = "dedicated"  -> creates tracer-{env}-valkey, its security group, its
...
# module seeded with product = "tracer" would produce tracer-{env}-vpc and
# tracer-{env}-eks, which do not exist.
################################################################################
...
################################################################################
# Valkey — tracer-{environment}-valkey
# Secret: tracer-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
