...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"tracer\" by validation: the derived name (tracer-{env}-valkey) and the derived secret path (tracer-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the tracer Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "tracer"

  validation {
    condition     = var.product == "tracer"
    error_message = "The product must be \"tracer\". This root stack is the tracer Valkey datastore; every name and secret path it produces is derived from it. To provision Valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# "lerian" product label — not "tracer", and not "shared" either (that one
# labels the shared DATASTORE tier in products/shared-resources). Deriving them
# from module.naming here would produce tracer-{env}-vpc /
# tracer-{env}-eks, which do not exist — which is why this root stack does
...
# reason: the tracer workloads run on the EKS nodes, and the replication group has to be
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"tracer\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "port" {
  description = "Port the cache nodes accept connections on. The tracer chart keeps host and port SPLIT — MULTI_TENANT_REDIS_HOST is a BARE hostname and MULTI_TENANT_REDIS_PORT carries the port (templates/configmap.yaml:62-63). This is the opposite of the midaz chart, whose REDIS_HOST embeds \"host:port\" in one string. Do not copy the midaz shape here."
  type        = number
  default     = 6379
}
...
variable "transit_encryption_mode" {
  description = "Transit encryption mode: \"preferred\" accepts both TLS and plaintext clients, \"required\" accepts TLS only. This value is what the helm_values output turns into MULTI_TENANT_REDIS_TLS. Note the disagreement worth knowing about: the tracer chart DEFAULTS MULTI_TENANT_REDIS_TLS to \"true\" (values.yaml:251) while this stack defaults the mode to \"preferred\" and therefore reports \"false\". The Terraform value describes what the server actually enforces and is the one to trust."
  type        = string
  default     = "preferred"
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Defaults to false so a first apply cannot lock the registry out. Unlike the midaz chart, tracer DOES ship the client-side knob — MULTI_TENANT_REDIS_PASSWORD is an optional chart secret (templates/secrets.yaml:6: \"MULTI_TENANT_REDIS_PASSWORD is optional (Redis without auth is allowed)\") — so enabling it is a tfvars change plus a values change, with no chart change needed. Rehearse it in stg first."
  type        = bool
  default     = false
}
