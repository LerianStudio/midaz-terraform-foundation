...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"tracer\" by validation: the derived name (tracer-{env}-postgres) and the derived secret path (tracer-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the tracer Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "tracer"

  validation {
    condition     = var.product == "tracer"
    error_message = "The product must be \"tracer\". This root stack is the tracer PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# reason: the tracer workloads run on the EKS nodes, and the instance has to be
...
variable "database_name" {
  description = "Name of the initial database created on the instance. The tracer chart reads it as DB_NAME and defaults it to \"tracer\" (values.yaml:204), so the two agree out of the box. tracer runs a SINGLE logical database, unlike midaz — helm_values can therefore emit DB_NAME, which the midaz root deliberately cannot."
  type        = string
  default     = "tracer"
}
...
variable "create_read_replica" {
  description = "Create a read replica, tracer-{env}-postgres-replica, published as the replica_endpoint output."
  type        = bool
  default     = false
}
