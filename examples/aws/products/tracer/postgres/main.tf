...
# the PostgreSQL datastore of the tracer product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/tracer/postgres/terraform.tfstate). Its sibling —
...
#
#   mode = "dedicated"  -> creates tracer-{env}-postgres, its security group, its
...
################################################################################
# PostgreSQL — tracer-{environment}-postgres
# Secret: tracer-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
