...
variable "product" {
  description = "Product this cluster belongs to. Pinned to \"plugin-br-pix-indirect-btg\" by validation: the derived name (plugin-br-pix-indirect-btg-{env}-docdb) and the derived secret path (plugin-br-pix-indirect-btg-{env}-docdb/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-pix-indirect-btg Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-br-pix-indirect-btg"

  validation {
    condition     = var.product == "plugin-br-pix-indirect-btg"
    error_message = "The product must be \"plugin-br-pix-indirect-btg\". This root stack is the plugin-br-pix-indirect-btg documentdb datastore; every name and secret path it produces is derived from it. To provision documentdb for another product, copy this directory to examples/aws/products/<product>/documentdb instead."
  }
}
...
# reason: the plugin-br-pix-indirect-btg workloads run on the EKS nodes, and the cluster has to be
...
variable "master_username" {
  description = "Master username of the cluster, emitted as MONGO_USER. The chart default is \"pix-btg\" and the bundled subchart provisions rootUser \"pix_btg\" (hyphen versus underscore — another internal disagreement). Neither is imposed here: this is the RDS/DocumentDB master account, it is emitted explicitly, and the Lerian default \"docdbadmin\" keeps it consistent with the other products. Deliberately NOT marked sensitive at this level: the module marks its own copy sensitive, and reading it back through that output would redact the whole helm_values map. In mode = \"shared\" this is therefore the value the CALLER declares, not a read of the shared cluster."
  type        = string
  default     = "docdbadmin"
}
