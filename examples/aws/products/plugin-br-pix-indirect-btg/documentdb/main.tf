################################################################################
# products/plugin-br-pix-indirect-btg/documentdb — the MongoDB-compatible datastore of plugin-br-pix-indirect-btg
#
# Message and event persistence for pix, inbound and outbound. The chart bundles a
# Bitnami mongodb subchart enabled by default and marks
# <component>.configmap.MONGO_HOST REQUIRED on the external path.
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-br-pix-indirect-btg/documentdb/terraform.tfstate). Its siblings —
...
#
#   mode = "dedicated"  -> creates plugin-br-pix-indirect-btg-{env}-docdb, its security group,
...
################################################################################
# DocumentDB — plugin-br-pix-indirect-btg-{environment}-docdb
# Secret: plugin-br-pix-indirect-btg-{env}-docdb/password   Host: the raw docdb writer endpoint
################################################################################
