...
variable "product" {
  description = "Product this instance belongs to. Pinned to \"plugin-br-pix-indirect-btg\" by validation: the derived name (plugin-br-pix-indirect-btg-{env}-postgres) and the derived secret path (plugin-br-pix-indirect-btg-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-pix-indirect-btg Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-br-pix-indirect-btg"

  validation {
    condition     = var.product == "plugin-br-pix-indirect-btg"
    error_message = "The product must be \"plugin-br-pix-indirect-btg\". This root stack is the plugin-br-pix-indirect-btg postgres datastore; every name and secret path it produces is derived from it. To provision postgres for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# reason: the plugin-br-pix-indirect-btg workloads run on the EKS nodes, and the instance has to be
...
variable "database_name" {
  description = "Name of the initial database created on the instance, emitted as DB_NAME for every component. THE CHART DISAGREES WITH ITSELF ABOUT THIS VALUE: pix, inbound and outbound default DB_NAME to \"pix\", reconciliation defaults it to \"pix_btg\", and the bundled postgresql subchart provisions \"pix_btg\". RDS creates exactly ONE initial database, so helm_values points all four components at this one name — otherwise reconciliation would dial a database that does not exist. Reported as a chart finding; see the product README."
  type        = string
  default     = "pix"
}

variable "username" {
  description = "Master username of the instance, emitted as DB_USER. The chart default is \"plugin\" for all four components while the bundled subchart provisions \"pix_btg\" — another internal disagreement. This default follows the application side, which is the half that has to match at runtime."
  type        = string
  default     = "plugin"
}
...
variable "create_read_replica" {
  description = "Create a read replica, plugin-br-pix-indirect-btg-{env}-postgres-replica, published as the replica_endpoint output. The chart carries a DB_REPLICA_* block on all four data-plane components and DEFAULTS IT TO THE PRIMARY, so helm_values always emits those keys and simply points them at the replica when one exists — the midaz shape, not the plugin-br-bank-transfer shape."
  type        = bool
  default     = false
}
