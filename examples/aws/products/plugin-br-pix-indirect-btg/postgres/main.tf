################################################################################
# products/plugin-br-pix-indirect-btg/postgres — the PostgreSQL datastore of the plugin-br-pix-indirect-btg product
#
# Pix message state for all four data-plane components. The chart bundles a
# Bitnami postgresql subchart enabled by default; on the external path it marks
# <component>.configmap.DB_HOST as REQUIRED and fails the render without it.
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-br-pix-indirect-btg/postgres/terraform.tfstate). Its siblings —
# documentdb, valkey — are independent roots with independent state,
# so a cache resize can never queue behind an RDS apply and a corrupt state takes
# down one datastore instead of three.
#
#   mode = "dedicated"  -> creates plugin-br-pix-indirect-btg-{env}-postgres, its security group,
#                          its Secrets Manager entry. This is the default.
...
# module seeded with product = "plugin-br-pix-indirect-btg" would produce plugin-br-pix-indirect-btg-{env}-vpc and
# plugin-br-pix-indirect-btg-{env}-eks, which do not exist.
################################################################################
...
# the documentdb, valkey siblings of this one.
#
...
################################################################################
# PostgreSQL — plugin-br-pix-indirect-btg-{environment}-postgres
# Secret: plugin-br-pix-indirect-btg-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
