################################################################################
# products/plugin-br-pix-indirect-btg/valkey — the cache datastore of the plugin-br-pix-indirect-btg product
#
# Cache and locks for pix, outbound and reconciliation. inbound and schedule have
# no Redis configuration. The chart bundles a valkey.io subchart (NOT Bitnami —
# its Service naming differs, which is why the chart carries its own host helper).
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-br-pix-indirect-btg/valkey/terraform.tfstate). Its siblings —
# postgres, documentdb — are independent roots with independent state,
# so a cache resize can never queue behind an RDS apply and a corrupt state takes
# down one datastore instead of three.
#
#   mode = "dedicated"  -> creates plugin-br-pix-indirect-btg-{env}-valkey, its security group,
...
# the postgres, documentdb siblings of this one.
#
...
#
# The "nothing can reach this replication group at all" case is not asserted anywhere in
...
################################################################################
# Valkey — plugin-br-pix-indirect-btg-{environment}-valkey
# Secret: plugin-br-pix-indirect-btg-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
