...
variable "product" {
  description = "Product this replication group belongs to. Pinned to \"plugin-br-pix-indirect-btg\" by validation: the derived name (plugin-br-pix-indirect-btg-{env}-valkey) and the derived secret path (plugin-br-pix-indirect-btg-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-pix-indirect-btg Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-br-pix-indirect-btg"

  validation {
    condition     = var.product == "plugin-br-pix-indirect-btg"
    error_message = "The product must be \"plugin-br-pix-indirect-btg\". This root stack is the plugin-br-pix-indirect-btg valkey datastore; every name and secret path it produces is derived from it. To provision valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# "lerian" product label — not "plugin-br-pix-indirect-btg", and not "shared" either (that one
# labels the shared DATASTORE tier in products/shared-resources). Deriving them
# from module.naming here would produce plugin-br-pix-indirect-btg-{env}-vpc /
# plugin-br-pix-indirect-btg-{env}-eks, which do not exist — which is why this root stack does
...
# reason: the plugin-br-pix-indirect-btg workloads run on the EKS nodes, and the replication group has to be
...
variable "allowed_security_group_ids" {
  description = "Extra security group IDs allowed to reach the replication group. Merged with the EKS node security group when eks_node_security_group_lookup_enabled is true."
  type        = list(string)
  default     = []
}

variable "allowed_cidr_blocks" {
  description = "Extra CIDR blocks allowed to reach the replication group. Merged with the private subnet CIDRs when allow_private_subnet_cidr_ingress is true."
  type        = list(string)
  default     = []
}
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"plugin-br-pix-indirect-btg\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "port" {
  description = "Port the cache nodes accept connections on. THE CHART IS INCONSISTENT ABOUT HOW THIS IS DELIVERED: pix and outbound take a separate REDIS_PORT key, while reconciliation has no REDIS_PORT at all and expects the port folded into REDIS_HOST as \"host:6379\". helm_values emits both shapes, one per component. See the outputs file."
  type        = number
  default     = 6379
}
...
variable "transit_encryption_mode" {
  description = "Transit encryption mode: \"preferred\" accepts both TLS and plaintext clients, \"required\" accepts TLS only. Keep it preferred until the chart is wired for TLS: REDIS_TLS defaults to false and REDIS_CA_CERT to empty on every component, so nothing mounts the ElastiCache CA bundle yet. Flipping this to \"required\" without that change locks all three Redis-speaking components out."
  type        = string
  default     = "preferred"
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Defaults to false because REDIS_PASSWORD is operator-provided and empty by default on every component (_helpers.tpl says so explicitly: REDIS_PASSWORD stays operator-provided) — turning enforcement on without wiring it locks every consumer out."
  type        = bool
  default     = false
}
...
variable "redis_db_index" {
  description = "Logical Valkey database index the components use, emitted as REDIS_DB. Not an AWS setting — ElastiCache exposes 16 logical databases on every node and Terraform creates none of them — but it belongs in helm_values so the chart wiring is complete in one place. The chart default is \"0\" on all three components that speak Redis."
  type        = number
  default     = 0
}
