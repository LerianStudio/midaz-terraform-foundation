...
variable "product" {
  description = "Product this instance belongs to. Pinned to \"plugin-br-bank-transfer\" by validation: the derived name (plugin-br-bank-transfer-{env}-postgres) and the derived secret path (plugin-br-bank-transfer-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-bank-transfer Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-br-bank-transfer"

  validation {
    condition     = var.product == "plugin-br-bank-transfer"
    error_message = "The product must be \"plugin-br-bank-transfer\". This root stack is the plugin-br-bank-transfer postgres datastore; every name and secret path it produces is derived from it. To provision postgres for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# reason: the plugin-br-bank-transfer workloads run on the EKS nodes, and the instance has to be
...
variable "database_name" {
  description = "Name of the initial database created on the instance, emitted as POSTGRES_DB. Note the chart key: this plugin says POSTGRES_DB where notifications says POSTGRES_NAME and midaz says DB_ONBOARDING_NAME — three charts, three spellings of the same idea. The chart default is \"bank_transfer\", matching the bundled subchart's postgresql.auth.database, which is why that is the default here."
  type        = string
  default     = "bank_transfer"
}

variable "username" {
  description = "Master username of the instance. The chart's POSTGRES_USER default is \"bank_transfer\" and so is the bundled subchart's postgresql.auth.username, so this default keeps the RDS master user and the application user the same account. Give the application its own least-privilege role once one exists and override POSTGRES_USER in the chart instead of changing this."
  type        = string
  default     = "bank_transfer"
}
...
variable "create_read_replica" {
  description = "Create a read replica, plugin-br-bank-transfer-{env}-postgres-replica, published as the replica_endpoint output. The chart carries a POSTGRES_REPLICA_* block for CQRS read scaling and renders it ONLY when POSTGRES_REPLICA_HOST is set (an if-guard in templates/configmap.yaml), so helm_values follows the same rule: the replica keys appear only when a replica exists."
  type        = bool
  default     = false
}
