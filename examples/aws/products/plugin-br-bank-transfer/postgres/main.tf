################################################################################
# products/plugin-br-bank-transfer/postgres — the PostgreSQL datastore of the plugin-br-bank-transfer product
#
# TED origination state, the idempotency ledger and the outbox. The chart bundles
# a Bitnami postgresql subchart in `architecture: replication` mode and enables it
# by default, so switching to RDS means setting BOTH postgresql.enabled = false
# AND postgresql.external = true — see the product README.
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-br-bank-transfer/postgres/terraform.tfstate). Its siblings —
...
#
#   mode = "dedicated"  -> creates plugin-br-bank-transfer-{env}-postgres, its security group,
...
# module seeded with product = "plugin-br-bank-transfer" would produce plugin-br-bank-transfer-{env}-vpc and
# plugin-br-bank-transfer-{env}-eks, which do not exist.
################################################################################
...
# the documentdb, valkey, rabbitmq siblings of this one.
#
...
################################################################################
# PostgreSQL — plugin-br-bank-transfer-{environment}-postgres
# Secret: plugin-br-bank-transfer-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
