################################################################################
# products/plugin-br-bank-transfer/valkey — the cache datastore of the plugin-br-bank-transfer product
#
# Idempotency keys, distributed locks and the rate limiter. The chart bundles a
# Bitnami valkey subchart enabled by default, with auth on.
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-br-bank-transfer/valkey/terraform.tfstate). Its siblings —
...
#
#   mode = "dedicated"  -> creates plugin-br-bank-transfer-{env}-valkey, its security group,
...
# the postgres, documentdb, rabbitmq siblings of this one.
#
...
################################################################################
# Valkey — plugin-br-bank-transfer-{environment}-valkey
# Secret: plugin-br-bank-transfer-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
