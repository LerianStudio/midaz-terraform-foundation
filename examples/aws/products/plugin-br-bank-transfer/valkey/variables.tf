...
variable "product" {
  description = "Product this replication group belongs to. Pinned to \"plugin-br-bank-transfer\" by validation: the derived name (plugin-br-bank-transfer-{env}-valkey) and the derived secret path (plugin-br-bank-transfer-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-bank-transfer Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-br-bank-transfer"

  validation {
    condition     = var.product == "plugin-br-bank-transfer"
    error_message = "The product must be \"plugin-br-bank-transfer\". This root stack is the plugin-br-bank-transfer valkey datastore; every name and secret path it produces is derived from it. To provision valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# "lerian" product label — not "plugin-br-bank-transfer", and not "shared" either (that one
# labels the shared DATASTORE tier in products/shared-resources). Deriving them
# from module.naming here would produce plugin-br-bank-transfer-{env}-vpc /
# plugin-br-bank-transfer-{env}-eks, which do not exist — which is why this root stack does
...
# reason: the plugin-br-bank-transfer workloads run on the EKS nodes, and the replication group has to be
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"plugin-br-bank-transfer\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "port" {
  description = "Port the cache nodes accept connections on. THIS CHART HAS NO REDIS_PORT VARIABLE: REDIS_HOST carries host:port in one string, and the init container proves it by splitting the value on a colon (templates/deployment.yaml runs REDIS_PORT_NUM=$(echo \\\"$REDIS_HOST\\\" | cut -d: -f2) before nc). The port is therefore folded into REDIS_HOST by helm_values, and only the MULTI_TENANT_REDIS_* pair stays split."
  type        = number
  default     = 6379
}
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Defaults to false because the plugin has no AUTH-before-first-command path on a plaintext connection — turning it on without a chart change locks every consumer out."
  type        = bool
  default     = false
}
...
variable "redis_db_index" {
  description = "Logical Valkey database index the plugin uses, emitted as REDIS_DB. Not an AWS setting — ElastiCache exposes 16 logical databases on every node and Terraform creates none of them — but it belongs in helm_values so the chart wiring is complete in one place. The chart default is \"0\"."
  type        = number
  default     = 0
}
