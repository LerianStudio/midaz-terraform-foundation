################################################################################
# products/plugin-br-bank-transfer/rabbitmq — the message broker of the plugin-br-bank-transfer product
#
# OPTIONAL. Both switches that would turn this datastore on ship OFF: the chart's
# rabbitmq subchart is `enabled: false` and the application's own
# RABBITMQ_ENABLED defaults to "false". Applying this directory is a decision to
# use the plugin's event bus, and it is the most expensive datastore of the four
# (~USD 100/month floor). Read the README before you do.
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-br-bank-transfer/rabbitmq/terraform.tfstate). Its siblings —
# postgres, documentdb, valkey — are independent roots with independent state.
# An AmazonMQ broker replacement is the slowest apply of the four, and per-service
# state means it never blocks an RDS parameter change.
#
#   mode = "dedicated"  -> creates plugin-br-bank-transfer-{env}-rabbitmq[-single|-cluster],
...
# the postgres, documentdb, valkey siblings of this one.
#
...
################################################################################
# RabbitMQ — plugin-br-bank-transfer-{environment}-rabbitmq[-single|-cluster]
# Secret: plugin-br-bank-transfer-{env}-rabbitmq/password   Host: the raw AmazonMQ broker host
################################################################################
