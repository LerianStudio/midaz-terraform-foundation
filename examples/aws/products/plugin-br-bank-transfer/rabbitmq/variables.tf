...
variable "product" {
  description = "Product this broker belongs to. Pinned to \"plugin-br-bank-transfer\" by validation: the derived name (plugin-br-bank-transfer-{env}-rabbitmq[-single|-cluster]) and the derived secret path (plugin-br-bank-transfer-{env}-rabbitmq/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-bank-transfer Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-br-bank-transfer"

  validation {
    condition     = var.product == "plugin-br-bank-transfer"
    error_message = "The product must be \"plugin-br-bank-transfer\". This root stack is the plugin-br-bank-transfer rabbitmq datastore; every name and secret path it produces is derived from it. To provision rabbitmq for another product, copy this directory to examples/aws/products/<product>/rabbitmq instead."
  }
}
...
variable "shared_broker_name" {
  description = <<-EOT
    ...
    plugin-br-bank-transfer release consuming the shared broker there sets:
...
# reason: the plugin-br-bank-transfer workloads run on the EKS nodes, and the broker has to be
...
variable "port" {
  description = "AMQPS port the broker listens on. AmazonMQ for RabbitMQ exposes NO plaintext AMQP listener, so this is 5671 and there is no reason to change it. This chart has no port variable of its own — the port lives inside RABBITMQ_URL — so this value is published through the amqp_endpoint output rather than through helm_values."
  type        = number
  default     = 5671
}

variable "console_port" {
  description = "Port of the RabbitMQ management console and management HTTP API. AmazonMQ serves both over HTTPS on 443. This chart has no management-port variable and no health check against the broker, so the port is opened for operators rather than for the application."
  type        = number
  default     = 443
}

variable "enable_console_ingress" {
  description = "Also open console_port (443) to the resolved ingress sources, so the management console and the management HTTP API stay reachable from inside the VPC. Unlike the midaz and notifications charts, nothing in this chart calls the management API, so turning it off costs only the console. The exposure is bounded by the ingress allow list either way."
  type        = bool
  default     = true
}
...
variable "mq_admin_user" {
  description = "Administrator username of the broker. It becomes the user half of RABBITMQ_URL, which the operator assembles — this chart has no RABBITMQ_USER variable. Deliberately NOT marked sensitive at this level: the module marks its own copy sensitive, and reading it back through that output would redact anything it is merged into. In mode = \"shared\" this is therefore the value the CALLER declares, not a read of the shared broker — keep it equal to the admin user of the shared tier (both default to \"rabbitmqadmin\")."
  type        = string
