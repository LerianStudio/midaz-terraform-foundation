...
variable "product" {
  description = "Product this cluster belongs to. Pinned to \"plugin-br-bank-transfer\" by validation: the derived name (plugin-br-bank-transfer-{env}-docdb) and the derived secret path (plugin-br-bank-transfer-{env}-docdb/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-bank-transfer Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-br-bank-transfer"

  validation {
    condition     = var.product == "plugin-br-bank-transfer"
    error_message = "The product must be \"plugin-br-bank-transfer\". This root stack is the plugin-br-bank-transfer documentdb datastore; every name and secret path it produces is derived from it. To provision documentdb for another product, copy this directory to examples/aws/products/<product>/documentdb instead."
  }
}
...
# Ingress
#
# Same two-source model as products/shared-resources/documentdb, for the same
# reason: the plugin-br-bank-transfer workloads run on the EKS nodes, and the cluster has to be
...
variable "allowed_security_group_ids" {
  description = "Extra security group IDs allowed to reach the cluster. Merged with the EKS node security group when eks_node_security_group_lookup_enabled is true."
  type        = list(string)
  default     = []
}

variable "allowed_cidr_blocks" {
  description = "Extra CIDR blocks allowed to reach the cluster. Merged with the private subnet CIDRs when allow_private_subnet_cidr_ingress is true."
  type        = list(string)
  default     = []
}
...
variable "master_username" {
  description = "Master username of the cluster. It becomes the user half of MONGO_URI. Deliberately NOT marked sensitive at this level: the module marks its own copy sensitive, and reading it back through that output would redact the whole helm handoff. In mode = \"shared\" this is therefore the value the CALLER declares, not a read of the shared cluster — keep it equal to the master username of the shared tier (both default to \"docdbadmin\")."
  type        = string
  default     = "docdbadmin"
}
...
variable "documentdb_tls" {
  description = "Value of the DocumentDB \"tls\" cluster parameter: enabled or disabled. Safe to enable: the MONGO_URI this root emits always carries the raw *.docdb.amazonaws.com writer endpoint, which is exactly what the DocumentDB certificate covers, so hostname validation passes. Turning it on appends tls=true to the URI, and the chart must then supply MONGO_TLS_CA_CERT (the global RDS CA bundle, which Terraform does not distribute)."
  type        = string
  default     = "disabled"
...
variable "mongo_auth_source" {
  description = "Authentication database put in the authSource query parameter of MONGO_URI. \"admin\" is correct for a DocumentDB master user: DocumentDB authenticates the master against admin, not against the application database. The chart's own bundled-subchart URI uses authSource=admin too, so this matches on both paths."
  type        = string
  default     = "admin"
}
