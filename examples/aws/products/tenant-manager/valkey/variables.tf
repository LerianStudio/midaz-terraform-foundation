...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"tenant-manager\" by validation: the derived name (tenant-manager-{env}-valkey) and the derived secret path (tenant-manager-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the tenant-manager Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "tenant-manager"

  validation {
    condition     = var.product == "tenant-manager"
    error_message = "The product must be \"tenant-manager\". This root stack is the tenant-manager Valkey datastore; every name and secret path it produces is derived from it. To provision Valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# "lerian" product label — not "tenant-manager", and not "shared" either (that one
# labels the shared DATASTORE tier in products/shared-resources). Deriving them
# from module.naming here would produce tenant-manager-{env}-vpc /
# tenant-manager-{env}-eks, which do not exist — which is why this root stack does
...
# reason: the tenant-manager workloads run on the EKS nodes, and the replication group has to be
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"tenant-manager\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
