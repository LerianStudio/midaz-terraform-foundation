...
variable "product" {
  description = "Product these buckets belong to. Pinned to \"tenant-manager\" by validation: the derived bucket name (tenant-manager-{env}-{logical}-{account_id}) IS the cross-stack discovery contract, and the service reads it from MIGRATIONS_S3_BUCKET / CASDOOR_TEMPLATE_S3_BUCKET. A different value here silently produces a bucket the tenant-manager release never writes to."
  type        = string
  default     = "tenant-manager"

  validation {
    condition     = var.product == "tenant-manager"
    error_message = "The product must be \"tenant-manager\". To provision object storage for another product, copy this directory to examples/aws/products/<product>/s3 instead."
  }
}
...
# Buckets
#
# Keyed by LOGICAL name. The key is what MIGRATIONS_S3_BUCKET and CASDOOR_TEMPLATE_S3_BUCKET
# resolves against, and helm_values indexes "migrations" and "casdoor-templates"
...
variable "buckets" {
  ...
  validation {
    condition     = alltrue([for k in ["migrations", "casdoor-templates"] : contains(keys(var.buckets), k)])
    error_message = "The buckets map must contain both \"migrations\" (MIGRATIONS_S3_BUCKET) and \"casdoor-templates\" (CASDOOR_TEMPLATE_S3_BUCKET); helm_values indexes each by name. Additional buckets may be added alongside them."
  }
}
...
variable "irsa_enabled" {
  description = "Create the IAM role a tenant-manager pod assumes to reach the bucket, and resolve the cluster OIDC provider to do it. TRUE is the normal path and makes this root depend on infra-base/eks existing — a harder prerequisite than the datastore roots have, because the OIDC lookup is a singular data source that fails the plan rather than resolving to nothing. FALSE emits the per-bucket IAM policies only, for an EKS stack that attaches them to a role it already manages."
  type        = bool
  default     = true
}
...
variable "service_account" {
  description = "Kubernetes service account allowed to assume the IRSA role, in \"namespace:name\" form (the notation examples/aws/infra-base/eks/iam.tf already uses). The default matches the tenant-manager chart's ServiceAccount when it is installed into the \"tenant-manager\" namespace with the default name; override it when either differs, because a mismatch produces a role no pod can assume and the failure surfaces at runtime as AccessDenied, not at apply time."
  type        = string
  ...
  validation {
    ...
    error_message = "The service_account must be in \"namespace:name\" form, e.g. \"platform:tenant-manager\"."
  }
}
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose OIDC provider is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"tenant-manager\"."
  type        = string
  default     = ""
}
