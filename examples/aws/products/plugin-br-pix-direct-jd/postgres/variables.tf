...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"plugin-br-pix-direct-jd\" by validation: the derived name (plugin-br-pix-direct-jd-{env}-postgres) and the derived secret path (plugin-br-pix-direct-jd-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-pix-direct-jd Helm release cannot find."
  type        = string
  default     = "plugin-br-pix-direct-jd"

  validation {
    condition     = var.product == "plugin-br-pix-direct-jd"
    error_message = "The product must be \"plugin-br-pix-direct-jd\". This root stack is the plugin-br-pix-direct-jd PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# "lerian" product label — not "plugin-br-pix-direct-jd", and not "shared"
# either (that one labels the shared DATASTORE tier).
################################################################################
...
variable "allow_private_subnet_cidr_ingress" {
  description = "Allow the CIDR blocks of the Type=private subnets to reach the instance. This is the DEFAULT ingress path and the reason this stack can be applied before infra-base/eks exists. It is strictly tighter than the VPC-CIDR fallback the module ships, which additionally covers the public subnets."
  type        = bool
  default     = true
}

variable "eks_node_security_group_lookup_enabled" {
  description = "Resolve the EKS node security group by tag and allow it on the instance. Safe to leave true before infra-base/eks exists: the lookup uses data \"aws_security_groups\" (PLURAL), which returns an empty list rather than failing the plan. It authorises BOTH workloads of this chart — the pix Deployment and the CronJob — since they run on the same nodes."
  type        = bool
  default     = true
}
...
variable "database_name" {
  description = "Name of the initial database created on the instance, emitted as the chart's DATABASE_NAME (and its docker-compose-compat twin POSTGRES_DB). Defaults to \"pix\", which is the chart default. This chart ships NO bootstrap Job — with the bundled Bitnami subchart the database is created by the container entrypoint from POSTGRES_DB, and with external RDS nothing would create it, so Terraform creating it here closes a real gap."
  type        = string
  default     = "pix"
}

variable "username" {
  description = "Master username of the instance, emitted as the chart's DATABASE_USER (and POSTGRES_USER). Kept as \"postgres\": the RDS master is the only role Terraform creates and this chart has no bootstrap Job to create a least-privilege one. The chart's own default is \"pix\"; if a DBA creates that role, override DATABASE_USER in the release rather than changing this value, which would rename the RDS master and force a replacement."
  type        = string
  default     = "postgres"
}
...
#
# There is deliberately NO read replica block here — see the comment in main.tf.
# The chart defines no replica environment variable, so a replica would be an
# RDS instance nothing could be pointed at.
################################################################################
