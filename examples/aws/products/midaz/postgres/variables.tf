...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"midaz\" by validation: the derived name (midaz-{env}-postgres) and the derived secret path (midaz-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the midaz Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "midaz"

  validation {
    condition     = var.product == "midaz"
    error_message = "The product must be \"midaz\". This root stack is the midaz PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# midaz workloads run on the EKS nodes, and the datastore has to be reachable
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"midaz\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "database_name" {
  description = "Name of the initial database created on the instance. midaz runs TWO logical databases on it, onboarding and transaction (see DB_ONBOARDING_NAME / DB_TRANSACTION_NAME in the chart); RDS creates exactly one at provisioning time, so the second is created by the application migration, not by Terraform."
  type        = string
  default     = "midaz"
}
...
variable "create_read_replica" {
  description = "Create a read replica, midaz-{env}-postgres-replica, published as the replica_endpoint output."
  type        = bool
  default     = false
}
