################################################################################
# products/midaz/rabbitmq — the message broker of the midaz product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/midaz/rabbitmq/terraform.tfstate). Its siblings —
# postgres, documentdb, valkey — are independent roots with independent state.
# That matters most here: an AmazonMQ broker replacement is the slowest apply of
# the four, and per-service state means it never blocks an RDS parameter change.
#
#   mode = "dedicated"  -> creates midaz-{env}-rabbitmq[-single|-cluster], its
...
################################################################################
# RabbitMQ — midaz-{environment}-rabbitmq[-single|-cluster]
# Secret: midaz-{env}-rabbitmq/password   Host: the raw AmazonMQ broker host
################################################################################
...
module "rabbitmq" {
  ...
  # console is opened by default because the ledger's RABBITMQ_HEALTH_CHECK_URL
