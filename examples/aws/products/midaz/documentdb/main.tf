################################################################################
# products/midaz/documentdb — the MongoDB-compatible datastore of midaz
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/midaz/documentdb/terraform.tfstate). Its siblings —
# postgres, valkey, rabbitmq — are independent roots with independent state.
#
#   mode = "dedicated"  -> creates midaz-{env}-docdb, its security group, its
...
################################################################################
# DocumentDB — midaz-{environment}-docdb
# Secret: midaz-{env}-docdb/password   Host: the raw docdb writer endpoint
################################################################################
