...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"midaz\" by validation: the derived name (midaz-{env}-valkey) and the derived secret path (midaz-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the midaz Helm release cannot find."
  type        = string
  default     = "midaz"

  validation {
    condition     = var.product == "midaz"
    error_message = "The product must be \"midaz\". This root stack is the midaz Valkey datastore; every name and secret path it produces is derived from it. To provision Valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# midaz workloads run on the EKS nodes, and the cache has to be reachable from
...
variable "transit_encryption_mode" {
  description = "Transit encryption mode: \"preferred\" accepts both TLS and plaintext clients, \"required\" accepts TLS only. Keep it preferred while the midaz chart still connects without TLS — this value is what the helm_values output turns into REDIS_TLS, and flipping it to \"required\" without a chart change locks the ledger out."
  type        = string
  default     = "preferred"
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Defaults to false because the midaz chart does not yet ship a Valkey TLS/AUTH client configuration — turning it on without that change locks every consumer out."
  type        = bool
  default     = false
}
...
variable "redis_db_index" {
  description = "Logical Valkey database index the ledger uses, emitted as REDIS_DB. Not an AWS setting — ElastiCache exposes 16 logical databases on every node and Terraform creates none of them — but it belongs in helm_values so the chart wiring is complete in one place."
  type        = number
  default     = 0
}
