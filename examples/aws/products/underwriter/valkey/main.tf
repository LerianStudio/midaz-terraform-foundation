...
# the Valkey datastore of the underwriter (lender) product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/underwriter/valkey/terraform.tfstate). Its sibling —
...
#
#   mode = "dedicated"  -> creates underwriter-{env}-valkey, its security group, its
...
################################################################################
# Valkey — underwriter-{environment}-valkey
# Secret: underwriter-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
