################################################################################
# products/notifications/valkey — the cache datastore of the notifications product
#
# notifications uses Valkey for the idempotency store, the destination-auth cache
# and the rate limiter. All four components read the same REDIS_* block.
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/notifications/valkey/terraform.tfstate). Its siblings —
# postgres, rabbitmq — are independent roots with independent state,
# so a cache resize never blocks an RDS parameter change.
#
#   mode = "dedicated"  -> creates notifications-{env}-valkey, its security group,
...
# the postgres, rabbitmq siblings of this one.
#
...
################################################################################
# Valkey — notifications-{environment}-valkey
# Secret: notifications-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
