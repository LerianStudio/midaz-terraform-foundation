...
variable "product" {
  description = "Product this replication group belongs to. Pinned to \"notifications\" by validation: the derived name (notifications-{env}-valkey) and the derived secret path (notifications-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the notifications Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "notifications"

  validation {
    condition     = var.product == "notifications"
    error_message = "The product must be \"notifications\". This root stack is the notifications valkey datastore; every name and secret path it produces is derived from it. To provision valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# reason: the notifications workloads run on the EKS nodes, and the replication group has to be
...
variable "port" {
  description = "Port the cache nodes accept connections on. The notifications chart keeps REDIS_PORT as its own variable (values.yaml config: REDIS_PORT: \\\"6379\\\"), so this is emitted SPLIT from REDIS_HOST. Do not copy the midaz shape here — that chart removed REDIS_PORT in its 3.0 and folds the port into REDIS_HOST; this one does not."
  type        = number
  default     = 6379
}
...
variable "transit_encryption_mode" {
  description = "Transit encryption mode: \"preferred\" accepts both TLS and plaintext clients, \"required\" accepts TLS only. Keep it preferred until the chart is wired for TLS: notifications puts REDIS_TLS and REDIS_CA_CERT in .Values.secrets and ships both empty, so nothing mounts the ElastiCache CA bundle yet. Flipping this to \"required\" without that change locks every component out."
  type        = string
  default     = "preferred"
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Defaults to false because the chart's REDIS_PASSWORD ships empty and no component sends AUTH on a plaintext connection — turning it on without that change locks every consumer out."
  type        = bool
  default     = false
}
...
variable "redis_db_index" {
  description = "Logical Valkey database index the service uses, emitted as REDIS_DB. Not an AWS setting — ElastiCache exposes 16 logical databases on every node and Terraform creates none of them — but it belongs in helm_values so the chart wiring is complete in one place. The chart default is \"0\"."
  type        = number
  default     = 0
}
