...
variable "product" {
  description = "Product this instance belongs to. Pinned to \"notifications\" by validation: the derived name (notifications-{env}-postgres) and the derived secret path (notifications-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the notifications Helm release cannot find, with no error. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "notifications"

  validation {
    condition     = var.product == "notifications"
    error_message = "The product must be \"notifications\". This root stack is the notifications postgres datastore; every name and secret path it produces is derived from it. To provision postgres for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# reason: the notifications workloads run on the EKS nodes, and the instance has to be
...
variable "database_name" {
  description = "Name of the initial database created on the instance. Unlike midaz, notifications runs a SINGLE logical database, so RDS creating it at provisioning time is the whole story and helm_values can emit POSTGRES_NAME with confidence. The chart default for POSTGRES_NAME is \"notifications\" (values.yaml config block), which is why that is the default here — keep the two in step or override both."
  type        = string
  default     = "notifications"
}

variable "username" {
  description = "Master username of the instance. The chart's POSTGRES_USER default is \"notifications\", so this default makes the RDS master user and the application user the same account. Give the application its own least-privilege role once one exists and override POSTGRES_USER in the chart instead of changing this."
  type        = string
  default     = "notifications"
}
...
variable "create_read_replica" {
  description = "Create a read replica, notifications-{env}-postgres-replica, published as the replica_endpoint output. The chart carries a full POSTGRES_REPLICA_* set (HOST/PORT/USER/PASSWORD/NAME/SSLMODE) and the service opens a second connection pool from it, so this is worth turning on wherever read traffic matters. Note that the chart routes those variables through .Values.secrets, not .Values.config — see the helm_secret_values output."
  type        = bool
  default     = false
}
