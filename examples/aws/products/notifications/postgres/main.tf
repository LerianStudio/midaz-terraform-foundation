################################################################################
# products/notifications/postgres — the PostgreSQL datastore of the notifications product
#
# The notifications service, its three workers (email, sms, webhook) and the
# golang-migrate Job all read ONE ConfigMap and ONE Secret, so there is a single
# set of POSTGRES_* variables to fill in — not one per component.
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/notifications/postgres/terraform.tfstate). Its siblings —
# valkey, rabbitmq — are independent roots with independent state,
# so a broker change can never queue behind an RDS apply and a corrupt state takes
# down one datastore instead of three.
#
#   mode = "dedicated"  -> creates notifications-{env}-postgres, its security group,
...
# module seeded with product = "notifications" would produce notifications-{env}-vpc and
# notifications-{env}-eks, which do not exist.
################################################################################
...
# the valkey, rabbitmq siblings of this one.
#
...
################################################################################
# PostgreSQL — notifications-{environment}-postgres
# Secret: notifications-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
