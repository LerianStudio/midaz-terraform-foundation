...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"plugin-fees\" by validation: the derived name (plugin-fees-{env}-docdb) and the derived secret path (plugin-fees-{env}-docdb/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-fees Helm release cannot find. Note the directory and the label keep the chart's own \"plugin-\" prefix — the name in this repository is the name of the chart, never a shortened form."
  type        = string
  default     = "plugin-fees"

  validation {
    condition     = var.product == "plugin-fees"
    error_message = "The product must be \"plugin-fees\". This root stack is the plugin-fees DocumentDB datastore; every name and secret path it produces is derived from it. To provision DocumentDB for another product, copy this directory to examples/aws/products/<product>/documentdb instead."
  }
}
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"plugin-fees\". The lookup matches by tag:Name = \"{cluster}-node\"."
  type        = string
  default     = ""
}
...
variable "master_username" {
  description = "Master username of the cluster. Feeds MONGO_USER (the chart uses the UNSUFFIXED family — MONGO_HOST, MONGO_PORT, MONGO_USER — not midaz's MONGO_ONBOARDING_* / MONGO_TRANSACTION_* pair). Deliberately NOT marked sensitive at this level: the module marks its own copy sensitive, and reading it back through that output would redact the whole helm_values map and defeat the handoff. In mode = \"shared\" this is therefore the value the CALLER declares, not a read of the shared cluster — keep it equal to the shared tier's master username (both default to \"docdbadmin\"). The chart's own default is the least-privilege application user \"plugin-fees\", which its optional bootstrap Job creates; helm_values overrides it with the master, because that is the only user Terraform creates."
  type        = string
  default     = "docdbadmin"
}

variable "port" {
  description = "Port the cluster listens on. Emitted as MONGO_PORT, which the chart keeps SEPARATE from MONGO_HOST — nothing here is concatenated."
  type        = number
  default     = 27017
}
...
variable "documentdb_tls" {
  description = "Value of the DocumentDB \"tls\" cluster parameter: enabled or disabled. Safe to enable on the host side: helm_values always carries the raw *.docdb.amazonaws.com writer endpoint, which is exactly what the DocumentDB certificate covers, so hostname validation passes. Unlike product-console, this chart DOES have a MONGO_TLS_CA_CERT variable to mount the global RDS CA bundle into — Terraform does not distribute that bundle, so flipping this to \"enabled\" is a two-sided change."
  type        = string
  default     = "disabled"
...
variable "enabled_cloudwatch_logs_exports" {
  description = "DocumentDB log types exported to CloudWatch Logs. Empty in dev keeps the bill down; audit is what a regulator asks for in prd, and a fee engine is squarely in scope for that question."
  type        = list(string)
  default     = []
}
