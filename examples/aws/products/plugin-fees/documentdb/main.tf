################################################################################
# products/plugin-fees/documentdb — the only REQUIRED datastore of plugin-fees
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-fees/documentdb/terraform.tfstate). Its
# sibling msk/ is an independent root with independent state — and, unlike this
# one, it is OPTIONAL: streaming is off by default in the chart. See
# ../README.md.
#
#   mode = "dedicated"  -> creates plugin-fees-{env}-docdb, its security group,
...
################################################################################
# DocumentDB — plugin-fees-{environment}-docdb
