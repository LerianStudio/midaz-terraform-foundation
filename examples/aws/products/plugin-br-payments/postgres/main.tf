################################################################################
# products/plugin-br-payments/postgres — the ONLY datastore of plugin-br-payments
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-br-payments/postgres/terraform.tfstate).
#
# IT HAS NO SIBLINGS, AND THAT IS A DESIGN DECISION OF THE SERVICE, NOT A GAP IN
# THIS REPOSITORY. plugin-br-payments deliberately runs on PostgreSQL alone:
#
#   ADR-002  asynchronous publication goes through a TRANSACTIONAL OUTBOX table
#            in PostgreSQL rather than a message broker, so the state change and
#            the intent to publish commit together.
#   ADR-003  idempotency keys live in a PostgreSQL table with a unique
#            constraint rather than in Redis, so the guarantee is as durable as
#            the payment itself.
#
# There is therefore no valkey/, no rabbitmq/ and no msk/ directory here. Adding
# one would provision infrastructure the chart has no environment variable to
# consume. See ../README.md for the quoted ADR text.
#
#   mode = "dedicated"  -> creates plugin-br-payments-{env}-postgres, its
...
# module seeded with product = "plugin-br-payments" would produce
...
################################################################################
# PostgreSQL — plugin-br-payments-{environment}-postgres
