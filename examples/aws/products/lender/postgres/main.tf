...
# the PostgreSQL datastore of the lender product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/lender/postgres/terraform.tfstate). Its sibling —
...
#
#   mode = "dedicated"  -> creates lender-{env}-postgres, its security group, its
...
################################################################################
# PostgreSQL — lender-{environment}-postgres
# Secret: lender-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
