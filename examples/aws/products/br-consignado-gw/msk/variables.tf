...
variable "product" {
  description = "Product this stack belongs to. Pinned to \"br-consignado-gw\" by validation. Only meaningful in dedicated mode, which this stack should not be in: in the default shared mode nothing is named after this product, because the cluster resolved is shared-{env}-msk."
  type        = string
...
variable "vpc_name" {
  description = "tag:Name of the VPC the brokers are placed in. Leave empty (the default) to DERIVE \"lerian-{environment}-vpc\", which is what infra-base/vpc creates and exports as its vpc_name output. Both cross-stack names carry the \"lerian\" FOUNDATION label, not \"br-consignado-gw\" and not \"shared\"."
  type        = string
  default     = ""
}
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"br-consignado-gw\". The lookup matches by tag:Name = \"{cluster}-node\"."
  type        = string
  default     = ""
}
