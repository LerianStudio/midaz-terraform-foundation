...
# the PostgreSQL datastore of the br-consignado-gw product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/br-consignado-gw/postgres/terraform.tfstate). Its sibling —
...
#
#   mode = "dedicated"  -> creates br-consignado-gw-{env}-postgres, its security group, its
...
# module seeded with product = "br-consignado-gw" would produce br-consignado-gw-{env}-vpc and
# br-consignado-gw-{env}-eks, which do not exist.
################################################################################
...
################################################################################
# PostgreSQL — br-consignado-gw-{environment}-postgres
# Secret: br-consignado-gw-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
