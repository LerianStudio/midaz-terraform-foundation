...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"br-consignado-gw\" by validation: the derived name (br-consignado-gw-{env}-postgres) and the derived secret path (br-consignado-gw-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the br-consignado-gw Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "br-consignado-gw"

  validation {
    condition     = var.product == "br-consignado-gw"
    error_message = "The product must be \"br-consignado-gw\". This root stack is the br-consignado-gw PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# reason: the br-consignado-gw workloads run on the EKS nodes, and the instance has to be
...
variable "database_name" {
  description = "Name of the initial database created on the instance, emitted as POSTGRES_NAME. The chart ships NO default for it — values.yaml:59 is an empty string — so this value is authoritative rather than a mirror of a chart default. br-consignado-gw runs a single logical database, so helm_values can emit the name; the midaz root cannot, because midaz runs two."
  type        = string
  default     = "consignado"
}

variable "username" {
  description = "Master username of the instance. This is the RDS MASTER user — the only role that exists on a freshly provisioned instance — and it is what helm_values emits. Where the chart's own default differs, the chart default names a least-privilege application role that nothing in this repository creates; creating it is an out-of-band step, exactly like the second logical database on the midaz instance. The chart ships no default for POSTGRES_USER either (values.yaml:58 is an empty string), so nothing contradicts the master user here."
  type        = string
  default     = "postgres"
}
...
variable "create_read_replica" {
  description = "Create a read replica, br-consignado-gw-{env}-postgres-replica, published as the replica_endpoint output."
  type        = bool
  default     = false
}
