...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"br-consignado-gw\" by validation: the derived name (br-consignado-gw-{env}-valkey) and the derived secret path (br-consignado-gw-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the br-consignado-gw Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "br-consignado-gw"

  validation {
    condition     = var.product == "br-consignado-gw"
    error_message = "The product must be \"br-consignado-gw\". This root stack is the br-consignado-gw Valkey datastore; every name and secret path it produces is derived from it. To provision Valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# "lerian" product label — not "br-consignado-gw", and not "shared" either (that one
# labels the shared DATASTORE tier in products/shared-resources). Deriving them
# from module.naming here would produce br-consignado-gw-{env}-vpc /
# br-consignado-gw-{env}-eks, which do not exist — which is why this root stack does
...
# reason: the br-consignado-gw workloads run on the EKS nodes, and the replication group has to be
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"br-consignado-gw\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "port" {
  description = "Port the cache nodes accept connections on. The br-consignado-gw chart has NO REDIS_PORT variable at all — the port is embedded in REDIS_HOST as \"host:port\", the same shape as the midaz chart. The chart's own documented example is docs/UPGRADE-1.0.md:138, REDIS_HOST: \"redis.cache.svc.cluster.local:6379\"."
  type        = number
  default     = 6379
}
...
variable "transit_encryption_mode" {
  description = "Transit encryption mode: \"preferred\" accepts both TLS and plaintext clients, \"required\" accepts TLS only. Kept \"preferred\": the chart exposes a single REDIS_HOST string and NO TLS switch of any kind, so there is no way to tell the client to negotiate TLS and \"required\" would lock it out outright."
  type        = string
  default     = "preferred"
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Kept false because the chart exposes NO Redis password variable at all — REDIS_HOST (values.yaml:61) is the only Redis key it defines — so an enforced token has no way to reach the client."
  type        = bool
  default     = false
}
