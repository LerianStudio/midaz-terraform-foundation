...
# the Valkey datastore of the br-consignado-gw product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/br-consignado-gw/valkey/terraform.tfstate). Its sibling —
...
#
#   mode = "dedicated"  -> creates br-consignado-gw-{env}-valkey, its security group, its
...
################################################################################
# Valkey — br-consignado-gw-{environment}-valkey
# Secret: br-consignado-gw-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
