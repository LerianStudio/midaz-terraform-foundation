...
# the PostgreSQL datastore of the plugin-br-pix-jd product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-br-pix-jd/postgres/terraform.tfstate). This product has a
# single datastore root.
#
#   mode = "dedicated"  -> creates plugin-br-pix-jd-{env}-postgres, its security group, its
...
# module seeded with product = "plugin-br-pix-jd" would produce plugin-br-pix-jd-{env}-vpc and
# plugin-br-pix-jd-{env}-eks, which do not exist.
################################################################################
...
################################################################################
# PostgreSQL — plugin-br-pix-jd-{environment}-postgres
# Secret: plugin-br-pix-jd-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
