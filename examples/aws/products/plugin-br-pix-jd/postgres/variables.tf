...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"plugin-br-pix-jd\" by validation: the derived name (plugin-br-pix-jd-{env}-postgres) and the derived secret path (plugin-br-pix-jd-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-pix-jd Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-br-pix-jd"

  validation {
    condition     = var.product == "plugin-br-pix-jd"
    error_message = "The product must be \"plugin-br-pix-jd\". This root stack is the plugin-br-pix-jd PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# "lerian" product label — not "plugin-br-pix-jd", and not "shared" either (that one
# labels the shared DATASTORE tier in products/shared-resources). Deriving them
# from module.naming here would produce plugin-br-pix-jd-{env}-vpc /
# plugin-br-pix-jd-{env}-eks, which do not exist — which is why this root stack does
...
# reason: the plugin-br-pix-jd workloads run on the EKS nodes, and the instance has to be
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"plugin-br-pix-jd\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "create_read_replica" {
  description = "Create a read replica, plugin-br-pix-jd-{env}-postgres-replica, published as the replica_endpoint output."
  type        = bool
  default     = false
}
