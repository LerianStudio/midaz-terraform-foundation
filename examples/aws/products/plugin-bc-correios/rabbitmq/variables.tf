...
variable "product" {
  description = "Product this broker belongs to. Pinned to \"plugin-bc-correios\" by validation: the derived name (plugin-bc-correios-{env}-rabbitmq[-single|-cluster]) and the derived secret path (plugin-bc-correios-{env}-rabbitmq/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-bc-correios Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-bc-correios"

  validation {
    condition     = var.product == "plugin-bc-correios"
    error_message = "The product must be \"plugin-bc-correios\". This root stack is the plugin-bc-correios broker; every name and secret path it produces is derived from it. To provision this service for another product, copy this directory to examples/aws/products/<product>/rabbitmq instead."
  }
}
...
variable "shared_broker_name" {
  description = <<-EOT
    ...
    plugin-bc-correios release consuming the shared broker there sets:
...
# plugin-bc-correios workloads run on the EKS nodes, and the broker has to be
...
variable "enable_console_ingress" {
  description = "Also open console_port (443) to the resolved ingress sources. This chart's application never calls the management API — it has no RABBITMQ_HEALTH_CHECK_URL — but global.externalRabbitmqDefinitions runs a bootstrap Job against it to create the user, vhost and permissions, and the console is how an operator inspects the broker. Turning it off blocks both."
  type        = bool
  default     = true
}
