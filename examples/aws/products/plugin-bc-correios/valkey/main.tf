################################################################################
# products/plugin-bc-correios/valkey — the cache datastore of plugin-bc-correios
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-bc-correios/valkey/terraform.tfstate). Its siblings are
# independent roots with independent state.
#
#   mode = "dedicated"  -> creates plugin-bc-correios-{env}-valkey, its security group and
#                          its Secrets Manager auth token. This is the default.
#   mode = "shared"     -> creates NOTHING. Resolves the replication group owned
#                          by products/shared-resources/valkey by name:
#                          shared-{env}-valkey plus shared-{env}-valkey/auth-token.
#
...
#
# The "nothing can reach this replication group at all" case is not asserted
# anywhere in this stack: the valkey-elasticache module already carries check
...
################################################################################
# Valkey — plugin-bc-correios-{environment}-valkey
# Secret: plugin-bc-correios-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
