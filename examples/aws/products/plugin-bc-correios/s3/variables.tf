...
variable "product" {
  description = "Product these buckets belong to. Pinned to \"plugin-bc-correios\" by validation: the derived bucket name (plugin-bc-correios-{env}-{logical}-{account_id}) IS the cross-stack discovery contract, and the chart reads it from OBJECT_STORAGE_BUCKET. A different value here silently produces a bucket the plugin-bc-correios release never writes to."
  type        = string
  default     = "plugin-bc-correios"

  validation {
    condition     = var.product == "plugin-bc-correios"
    error_message = "The product must be \"plugin-bc-correios\". To provision object storage for another product, copy this directory to examples/aws/products/<product>/s3 instead."
  }
}
...
# resolves against, and helm_values indexes "bc-correios-attachments" specifically — which is
...
variable "buckets" {
  ...
  validation {
    condition     = contains(keys(var.buckets), "bc-correios-attachments")
    error_message = "The buckets map must contain the key \"bc-correios-attachments\": it is the bucket the plugin-bc-correios-helm chart expects in OBJECT_STORAGE_BUCKET, and helm_values indexes it by name. Additional buckets may be added alongside it."
  }
}
...
variable "irsa_enabled" {
  description = "Create the IAM role a plugin-bc-correios pod assumes to reach the bucket, and resolve the cluster OIDC provider to do it. TRUE is the normal path and makes this root depend on infra-base/eks existing — a harder prerequisite than the datastore roots have, because the OIDC lookup is a singular data source that fails the plan rather than resolving to nothing. FALSE emits the per-bucket IAM policies only, for an EKS stack that attaches them to a role it already manages."
  type        = bool
  default     = true
}
...
variable "service_account" {
  description = "Kubernetes service account allowed to assume the IRSA role, in \"namespace:name\" form (the notation examples/aws/infra-base/eks/iam.tf already uses). The default matches the plugin-bc-correios-helm chart's ServiceAccount when it is installed into the \"bc-correios\" namespace with the default name; override it when either differs, because a mismatch produces a role no pod can assume and the failure surfaces at runtime as AccessDenied, not at apply time."
  type        = string
  default     = "bc-correios:bc-correios"

  validation {
    condition     = var.service_account == "" || can(regex("^[a-z0-9]([a-z0-9-]*[a-z0-9])?:[a-z0-9]([a-z0-9.-]*[a-z0-9])?$", var.service_account))
    error_message = "The service_account must be in \"namespace:name\" form, e.g. \"bc-correios:bc-correios\"."
  }
}
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose OIDC provider is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"plugin-bc-correios\"."
  type        = string
  default     = ""
}
