################################################################################
# products/plugin-bc-correios/s3 — the object storage of plugin-bc-correios
#
# ONE ROOT STACK PER SERVICE, same as the datastore siblings: one directory, one
# state file (aws/products/plugin-bc-correios/s3/terraform.tfstate).
#
...
################################################################################
# Object storage — plugin-bc-correios-{environment}-bc-correios-attachments-{account_id}
#
# The account id suffix is the documented naming exception: S3 bucket names are
# globally unique across every AWS account, so plugin-bc-correios-{env}-bc-correios-attachments alone
...
#
# bc-correios-attachments holds correspondence attachments exchanged with the
# Brazilian Central Bank. It is the only bucket this product needs, and the one
# with the longest retention of the three.
################################################################################
