# products/plugin-bc-correios

AWS datastores and object storage for the **plugin-bc-correios** product:
the Brazilian Central Bank mail system integration.

```
examples/aws/products/plugin-bc-correios/
├── postgres/     -> _modules/postgres-rds         plugin-bc-correios-{env}-postgres
├── valkey/       -> _modules/valkey-elasticache   plugin-bc-correios-{env}-valkey
├── rabbitmq/     -> _modules/rabbitmq-amazonmq    plugin-bc-correios-{env}-rabbitmq
└── s3/           -> _modules/s3-bucket            plugin-bc-correios-{env}-bc-correios-attachments-{account_id}
```
...
plugin-bc-correios:
  ...
  valkey:     yes
  rabbitmq:   yes
  redpanda:   no      # extras: seaweedfs/S3 (bucket bc-correios-attachments)
```

Confirmed against `Chart.yaml`, with one correction:

| Chart dependency | Version | Default | Turn off with |
| --- | --- | --- | --- |
| `postgresql` | 16.3.5 | **enabled** | `postgresql.enabled: false` + `postgresql.external: true` |
| `valkey` | 2.4.7 | **enabled** | `valkey.enabled: false` + `valkey.external: true` |
| `rabbitmq` | 2.1.11 | **enabled** | `rabbitmq.enabled: false` + `rabbitmq.external: true` |
| SeaweedFS | image `3.80` | **enabled** | `seaweedfs.enabled: false` |

> **SeaweedFS is not a subchart here.** It does not appear in `Chart.yaml` at
> all — the chart runs it from its own inline manifests
> (`templates/seaweedfs.yaml`). `seaweedfs.enabled: false` removes both the
> workload and the init container's wait for it.

The directory is named `plugin-bc-correios`, matching the chart directory exactly.
The `plugin-` prefix is part of the name, not decoration to be stripped: it is
what `product` is pinned to, and therefore what every resource name and secret
path derives from.
...
The naming difference is the whole mechanism — `plugin-bc-correios-dev-valkey` versus
...
| `plugin-bc-correios-` | this product's own datastores. |
...
5. products/plugin-bc-correios/*                       in any order, in parallel
6. helm upgrade --install plugin-bc-correios ...
```
...
A `dedicated` deployment can skip it entirely. For this product `shared` is worth
a hard look in dev: `mq.m7g.medium` alone is ~USD 100/month for a plugin that
...
```bash
cd examples/aws/products/plugin-bc-correios/<service>

terraform init \
  -backend-config=../../../backend/dev.hcl \
  -backend-config="key=aws/products/plugin-bc-correios/<service>/terraform.tfstate"
...
| Root | Fills in |
| --- | --- |
| `postgres` | `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_USER`, `POSTGRES_NAME`, `POSTGRES_SSLMODE` |
| `valkey` | `CACHE_ADDR` (as `host:port`) |
| `rabbitmq` | `RABBITMQ_HOST`, `RABBITMQ_USER` |
| `s3` | `OBJECT_STORAGE_PROVIDER`, `OBJECT_STORAGE_BUCKET`, `OBJECT_STORAGE_ENDPOINT`, `OBJECT_STORAGE_PATH_STYLE` |

**This product shares no variable names with any other.** Not `DB_*`, not
`REDIS_*`, not `MONGO_*`, and no `RABBITMQ_PORT_*` pair at all. Every key above
came from reading `templates/configmap.yaml`.

Three things that need attention before the first release:

1. **`POSTGRES_NAME` and `POSTGRES_USER` override illegal chart defaults.** Both
   default to `plugin-bc-correios` in the chart; RDS rejects hyphens in `db_name`
   and in the master username. This root uses `bc_correios` for both.
2. **`RABBITMQ_URL` is the real connection** and it is a Secret, because it
   embeds the password. Terraform emits only `RABBITMQ_HOST` and `RABBITMQ_USER`;
   the ExternalSecret assembles
   `amqps://<user>:<pass>@<endpoint>:5671/`.
3. **The RabbitMQ init container is a blocker.** See below.
...
The broker is 79% of the dev bill for a plugin whose messaging is not
high-volume. `mode = "shared"` on `rabbitmq` is the obvious dev configuration
once `products/shared-resources/rabbitmq` exists.
...
## Known blocker: the RabbitMQ init container cannot reach AmazonMQ

`templates/deployment.yaml` waits for the broker on a **hardcoded** port:
...
AmazonMQ for RabbitMQ serves **AMQPS on 5671** and publishes no plaintext
listener, so this TCP check can never succeed against a managed broker: the init
container retries for its full 300-second timeout, exits 1, and the pod never
reaches the application container.

**No Terraform value fixes this.** The literal has to become a configurable port
— a chart change. Until then, either keep the bundled in-cluster broker for this
product or patch the init container.

The `rabbitmq` root here is still correct and still worth applying: the broker,
its security group and its secret are real and the mapping is right. What should
not be scheduled is the Helm cutover.
