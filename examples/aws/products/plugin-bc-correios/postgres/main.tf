################################################################################
# products/plugin-bc-correios/postgres — the PostgreSQL datastore of plugin-bc-correios
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-bc-correios/postgres/terraform.tfstate). Its siblings are
# independent roots with independent state, so a RabbitMQ change can never queue
# behind an RDS apply and a corrupt state takes down one datastore instead of
# four.
#
#   mode = "dedicated"  -> creates plugin-bc-correios-{env}-postgres, its security group
#                          and its Secrets Manager entry. This is the default.
#   mode = "shared"     -> creates NOTHING. Resolves the instance owned by
#                          products/shared-resources/postgres by name:
#                          shared-{env}-postgres plus shared-{env}-postgres/password.
#
...
# is "database" and selects the subnets the instance is PLACED in; it
# goes to the postgres-rds module only.
#
# The "nothing can reach this instance at all" case is not asserted
# anywhere in this stack: the postgres-rds module already carries check
...
################################################################################
# PostgreSQL — plugin-bc-correios-{environment}-postgres
# Secret: plugin-bc-correios-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
