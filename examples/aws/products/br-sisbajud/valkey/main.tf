################################################################################
# products/br-sisbajud/valkey — the cache/lock datastore of the br-sisbajud product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/br-sisbajud/valkey/terraform.tfstate). Its siblings —
# postgres and msk — are independent roots with independent state.
#
# The chart calls this "Redis" everywhere (REDIS_HOST, REDIS_PASSWORD) and pins
# the Bitnami VALKEY subchart 2.4.7. Both are correct: valkey is the engine,
# redis is the wire protocol the client speaks. Do not "fix" either side.
#
#   mode = "dedicated"  -> creates br-sisbajud-{env}-valkey, its security group, its
...
################################################################################
# Valkey — br-sisbajud-{environment}-valkey
# Secret: br-sisbajud-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
