...
variable "product" {
  description = "Product this cluster belongs to. Pinned to \"br-sisbajud\" by validation: the derived name (br-sisbajud-{env}-msk) and the derived secret name (AmazonMSK_br-sisbajud-{env}-msk) ARE the cross-stack discovery contract. A different value here silently produces resources the br-sisbajud Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "br-sisbajud"

  validation {
    condition     = var.product == "br-sisbajud"
    error_message = "The product must be \"br-sisbajud\". This root stack is the br-sisbajud MSK cluster; every name and secret path it produces is derived from it. To provision Kafka for another product, copy this directory to examples/aws/products/<product>/msk instead."
  }
}

variable "mode" {
  description = "\"dedicated\" creates an MSK cluster owned by br-sisbajud (br-sisbajud-{env}-msk). \"shared\" creates nothing and resolves the cluster owned by products/shared-resources/msk, looked up by name: shared-{env}-msk plus the secret AmazonMSK_shared-{env}-msk. In shared mode this stack resolves no VPC, no subnets and no EKS security group, and plans to zero resources. SHARED IS THE NORMAL CHOICE: three brokers is the real floor for a dedicated cluster (~USD 105/month), and the only argument for paying it here is regulatory isolation of the SISBAJUD judicial-blocking event stream — a shared Kafka cluster is one topic namespace, one set of ACLs and one retention budget."
  type        = string
  default     = "dedicated"
...
# "lerian" product label — not "br-sisbajud", and not "shared" either (that one
# labels the shared DATASTORE tier in products/shared-resources).
################################################################################

variable "vpc_name" {
  description = "tag:Name of the VPC the brokers are placed in. Leave empty (the default) to DERIVE \"lerian-{environment}-vpc\", which is what infra-base/vpc creates and exports as its vpc_name output. Both cross-stack names carry the \"lerian\" FOUNDATION label, not \"br-sisbajud\" and not \"shared\"."
  type        = string
  default     = ""
}
...
variable "enable_sasl_scram" {
  description = "Enable SASL/SCRAM authentication. In dedicated mode the credentials land in Secrets Manager as AmazonMSK_br-sisbajud-{environment}-msk under a customer managed CMK; the AmazonMSK_ prefix and the CMK are both AWS requirements for a secret associated with a cluster, which is why this is the ONE Lerian datastore whose secret is not named {name}/password. In shared mode this decides whether the shared cluster's secret is resolved at all. Keep it true: MSK SASL/SCRAM is SCRAM-SHA-512 and it is what STREAMING_SASL_MECHANISM in the chart has to match."
  type        = bool
  default     = true
}

variable "scram_username" {
  description = "SASL/SCRAM username stored in Secrets Manager, emitted as STREAMING_SASL_USERNAME. Not a resource name, which is why it stays \"lerian\" rather than carrying the product label. The module creates ONE user; finer-grained users and ACLs are created against the cluster itself, outside Terraform."
  type        = string
  default     = "lerian"
}

variable "enable_unauthenticated" {
  description = "Allow unauthenticated Kafka access. Throwaway environments only. On a cluster carrying SISBAJUD judicial blocking events this removes the only application-level boundary there is, leaving the security group as the sole control."
  type        = bool
  default     = false
}
...
variable "auto_create_topics_enable" {
  description = "Kafka auto.create.topics.enable. Keep it FALSE. The br-sisbajud chart creates its six topics explicitly from an ArgoCD PreSync Job running `rpk topic create` (templates/topics/job.yaml, values.yaml:178-184), with partition count, replication factor and retention as chart values. Turning this on would let a typo in a topic name create a live topic with broker defaults instead of failing loudly."
  type        = bool
  default     = false
}

variable "default_replication_factor" {
  description = "Kafka default.replication.factor. Null derives min(number_of_broker_nodes, 3). This is the BROKER default and applies to topics created without an explicit factor; the chart passes its own through topics.replicationFactor, so the two should agree."
  type        = number
  default     = null
}
