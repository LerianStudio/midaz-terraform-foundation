################################################################################
# products/br-sisbajud/postgres — the PostgreSQL datastore of the br-sisbajud product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/br-sisbajud/postgres/terraform.tfstate). Its siblings —
# valkey and msk — are independent roots with independent state, so an MSK
# broker replacement can never queue behind an RDS parameter change and a
# corrupt state takes down one datastore instead of three.
#
# br-sisbajud has NO DocumentDB and NO RabbitMQ: chart 1.1.0 declares exactly two
# subcharts (postgresql 16.3.5, valkey 2.4.7, both condition-gated and both
# default OFF) and reaches Kafka through the STREAMING_* contract. There is no
# mongodb dependency and no AMQP variable anywhere in the chart.
#
#   mode = "dedicated"  -> creates br-sisbajud-{env}-postgres, its security group, its
...
# a naming module seeded with product = "br-sisbajud" would produce br-sisbajud-{env}-vpc and
# br-sisbajud-{env}-eks, which do not exist.
################################################################################
...
################################################################################
# PostgreSQL — br-sisbajud-{environment}-postgres
# Secret: br-sisbajud-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
