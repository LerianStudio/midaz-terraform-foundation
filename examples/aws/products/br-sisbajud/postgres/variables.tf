...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"br-sisbajud\" by validation: the derived name (br-sisbajud-{env}-postgres) and the derived secret path (br-sisbajud-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the br-sisbajud Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "br-sisbajud"

  validation {
    condition     = var.product == "br-sisbajud"
    error_message = "The product must be \"br-sisbajud\". This root stack is the br-sisbajud PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# br-sisbajud workloads run on the EKS nodes, and the datastore has to be reachable
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"br-sisbajud\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "database_name" {
  description = "Name of the initial database created on the instance. br-sisbajud runs ONE logical database, and the chart calls it br_sisbajud (brSisbajud.configmap.POSTGRES_NAME in values-template.yaml, and the golang-migrate Job's POSTGRES_NAME fallback default). UNDERSCORE, not hyphen: the AWS resource name is br-sisbajud-{env}-postgres but the database inside it is br_sisbajud, because PostgreSQL identifiers with a hyphen have to be quoted everywhere and the migrator does not quote them."
  type        = string
  default     = "br_sisbajud"
...
variable "username" {
  description = "Master username of the instance. Defaults to br_sisbajud rather than postgres, because NOTHING in the chart creates an application role: the migration Job connects as brSisbajud.configmap.POSTGRES_USER (chart default br_sisbajud) with brSisbajud.secrets.POSTGRES_PASSWORD, and that role has to already exist. Making the RDS master user the one the chart expects is what lets a fresh instance take the first migration. Create per-service least-privilege roles on the instance afterwards, outside Terraform, if the deployment needs them."
  type        = string
  default     = "br_sisbajud"
}
...
variable "create_read_replica" {
  description = "Create a read replica, br-sisbajud-{env}-postgres-replica, published as the replica_endpoint output."
  type        = bool
  default     = false
}
