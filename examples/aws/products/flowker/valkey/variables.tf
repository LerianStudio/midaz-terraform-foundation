...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"flowker\" by validation: the derived name (flowker-{env}-valkey) and the derived secret path (flowker-{env}-valkey/auth-token) ARE the cross-stack discovery contract."
  type        = string
  default     = "flowker"

  validation {
    condition     = var.product == "flowker"
    error_message = "The product must be \"flowker\". This root stack is the flowker Valkey datastore; every name and secret path it produces is derived from it. To provision Valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# "lerian" product label — not "flowker", and not "shared" either.
################################################################################
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"flowker\". The lookup matches by tag:Name = \"{cluster}-node\"."
  type        = string
  default     = ""
}
...
variable "engine_version" {
  description = "Valkey engine version. The vendored tarball in the flowker chart directory is valkey-helm 0.7.4 with appVersion 8.1.3, which is the in-cluster chart, not a constraint on ElastiCache — the ElastiCache Valkey engine line is what applies here."
  type        = string
  default     = "7.2"
}
...
variable "port" {
  description = "Port the cache nodes accept connections on. Whether flowker's client wants host and port split or joined is UNKNOWN — the midaz chart requires \"host:port\" in a single REDIS_HOST while plugin-fees splits MULTI_TENANT_REDIS_HOST and MULTI_TENANT_REDIS_PORT, and flowker has no readable chart to settle it. Both shapes are derivable from the endpoint and port outputs."
  type        = number
  default     = 6379
}
...
variable "transit_encryption_mode" {
  description = "Transit encryption mode: \"preferred\" accepts both TLS and plaintext clients, \"required\" accepts TLS only. Kept at \"preferred\" because there is no readable flowker chart to confirm the client speaks TLS, and \"required\" against a plaintext client locks the service out with a connection error that looks like a network problem."
  type        = string
  default     = "preferred"
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Defaults to false for the same reason as transit_encryption_mode: with no chart to read, there is no way to confirm the client sends AUTH, and turning it on without that locks every consumer out."
  type        = bool
  default     = false
}
