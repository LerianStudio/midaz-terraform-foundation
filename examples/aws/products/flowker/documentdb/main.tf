################################################################################
# products/flowker/documentdb — the MongoDB-compatible datastore of flowker
#
################################################################################
# THE COMPOSITION OF THIS PRODUCT IS INFERRED, NOT READ. READ ../README.md.
#
# flowker has NO readable Helm chart in this monorepo. The directory
...
# unambiguous evidence that flowker runs on MongoDB and Valkey, which is also
# what product-infra-dependencies.yaml records — while the CHART CONTRACT is
# not. Not one flowker-side environment variable name appears in any readable
...
# state file (aws/products/flowker/documentdb/terraform.tfstate). Its sibling
# valkey/ is an independent root with independent state.
#
#   mode = "dedicated"  -> creates flowker-{env}-docdb, its security group, its
...
################################################################################
# DocumentDB — flowker-{environment}-docdb
# Secret: flowker-{env}-docdb/password   Host: the raw docdb writer endpoint
################################################################################
