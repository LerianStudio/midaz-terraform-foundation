...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"flowker\" by validation: the derived name (flowker-{env}-docdb) and the derived secret path (flowker-{env}-docdb/password) ARE the cross-stack discovery contract."
  type        = string
  default     = "flowker"

  validation {
    condition     = var.product == "flowker"
    error_message = "The product must be \"flowker\". This root stack is the flowker DocumentDB datastore; every name and secret path it produces is derived from it. To provision DocumentDB for another product, copy this directory to examples/aws/products/<product>/documentdb instead."
  }
}
