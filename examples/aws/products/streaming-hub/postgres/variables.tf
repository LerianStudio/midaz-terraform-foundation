...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"streaming-hub\" by validation: the derived name (streaming-hub-{env}-postgres) and the derived secret path (streaming-hub-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the streaming-hub Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "streaming-hub"

  validation {
    condition     = var.product == "streaming-hub"
    error_message = "The product must be \"streaming-hub\". This root stack is the streaming-hub PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# reason: the streaming-hub workloads run on the EKS nodes, and the instance has to be
...
variable "database_name" {
  description = "Name of the initial database created on the instance. It MUST be \"streaminghub\": the auth-backend component hard-codes dbName: streaminghub as a literal in its streaming-hub Beego config (templates/auth-backend/configmap.yaml:10), where it is not templated from any values key and therefore cannot be overridden from Helm values. The auth component agrees (DB_NAME default \"streaminghub\", values.yaml:287), as does the bundled subchart (auth-database.auth.database, values.yaml:417). Changing this value breaks streaming-hub, not just the wiring."
  type        = string
  default     = "streaminghub"
}

variable "username" {
  description = "Master username of the instance. This is the RDS MASTER user — the only role that exists on a freshly provisioned instance — and it is what helm_values emits. Where the chart's own default differs, the chart default names a least-privilege application role that nothing in this repository creates; creating it is an out-of-band step, exactly like the second logical database on the midaz instance. Here the chart default is \"auth\" (values.yaml:284), the role the bundled Bitnami subchart creates; on RDS that role does not exist until someone creates it. Note also USER_EXECUTE_COMMAND, which the auth ConfigMap defaults to \"postgres\" (templates/auth/configmap.yaml:80)."
  type        = string
  default     = "postgres"
}
...
variable "create_read_replica" {
  description = "Create a read replica, streaming-hub-{env}-postgres-replica, published as the replica_endpoint output."
  type        = bool
  default     = false
}
