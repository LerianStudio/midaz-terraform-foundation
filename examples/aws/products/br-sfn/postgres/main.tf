################################################################################
# products/br-sfn/postgres — the PostgreSQL datastore of the br-sfn product
#
# br-sfn is the Brazilian SFN rails MONOREPO: SPB/STR (TED), SPI (Pix, four
# binaries), SILOC (card settlement), SCR (credit information), desk, correios
# (BC Correio regulatory mailbox), slc-edge and the cockpit SPA. Every component
# ships `enabled: false` and an environment enables only the rails it operates.
#
# ONE INSTANCE, SEVERAL RAIL DATABASES. Each rail that owns a schema reads its
# own POSTGRES_DB (correios reads POSTGRES_NAME) from its own component configmap
# and runs its own PreSync migration Job. RDS creates ONE database at
# provisioning time, so the per-rail databases are created on the instance
# outside Terraform — see outputs.tf and README.md.
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/br-sfn/postgres/terraform.tfstate). Its siblings —
# valkey, rabbitmq and msk — are independent roots with independent state, so a
# RabbitMQ change can never queue behind an RDS apply and a corrupt state takes
# down one datastore instead of four.
#
# br-sfn has NO DocumentDB: the chart declares no mongodb dependency and no
# MONGO_* variable exists anywhere in it.
#
#   mode = "dedicated"  -> creates br-sfn-{env}-postgres, its security group, its
...
# a naming module seeded with product = "br-sfn" would produce br-sfn-{env}-vpc and
...
################################################################################
# PostgreSQL — br-sfn-{environment}-postgres
# Secret: br-sfn-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
