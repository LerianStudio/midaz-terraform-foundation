...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"br-sfn\" by validation: the derived name (br-sfn-{env}-postgres) and the derived secret path (br-sfn-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the br-sfn Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "br-sfn"

  validation {
    condition     = var.product == "br-sfn"
    error_message = "The product must be \"br-sfn\". This root stack is the br-sfn PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# "lerian" product label — not "br-sfn", and not "shared" either (that one labels
# the shared DATASTORE tier in products/shared-resources). Deriving them from module.naming here would
# produce br-sfn-{env}-vpc / br-sfn-{env}-eks, which do not exist — which is why
...
# Ingress
#
# Same two-source model as products/shared-resources/postgres, for the same reason: the
# br-sfn workloads run on the EKS nodes, and the datastore has to be reachable
# from them on the first apply, before anyone has wired a security group by hand.
#
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"br-sfn\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "database_name" {
  description = "Name of the INITIAL database RDS creates. br-sfn is a MONOREPO OF RAILS and each rail that owns a schema reads its own database name from its own component configmap (POSTGRES_DB for spb/spi/siloc/scr/desk, POSTGRES_NAME for correios). RDS creates exactly ONE database at provisioning time, so this is the bootstrap database and the per-rail databases are created on the instance afterwards, outside Terraform. Underscore, not hyphen: an unquoted PostgreSQL identifier cannot contain a hyphen and the golang-migrate DSN the chart builds does not quote it."
  type        = string
  default     = "br_sfn"

  validation {
    condition     = can(regex("^[a-zA-Z_][a-zA-Z0-9_]*$", var.database_name))
    error_message = "The database_name must be an unquoted PostgreSQL identifier: letters, digits and underscores only, not starting with a digit. A hyphen would force every migration and every DSN to quote the name."
  }
}

variable "username" {
  description = "Master username of the instance. Nothing in the br-sfn chart creates a role: each rail's PreSync migration Job connects as its component's POSTGRES_USER with POSTGRES_PASSWORD, and that role must already exist. The master user created here is what bootstraps the rest. NOTE the chart's own constraint on the PASSWORD, which Terraform generates: it must be URL-SAFE, because the baked-flavour migration Jobs interpolate it into a postgres:// DSN unescaped (templates/_helpers.tpl:604) — see README.md."
  type        = string
  default     = "br_sfn"
}
...
variable "create_read_replica" {
  description = "Create a read replica, br-sfn-{env}-postgres-replica, published as the replica_endpoint output."
  type        = bool
  default     = false
}
