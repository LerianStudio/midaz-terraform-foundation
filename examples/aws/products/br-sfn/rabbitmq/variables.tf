...
variable "product" {
  description = "Product this broker belongs to. Pinned to \"br-sfn\" by validation: the derived name (br-sfn-{env}-rabbitmq[-single|-cluster]) and the derived secret path (br-sfn-{env}-rabbitmq/password) ARE the cross-stack discovery contract. A different value here silently produces resources the br-sfn Helm release cannot find."
  type        = string
  default     = "br-sfn"

  validation {
    condition     = var.product == "br-sfn"
    error_message = "The product must be \"br-sfn\". This root stack is the br-sfn RabbitMQ broker; every name and secret path it produces is derived from it. To provision a broker for another product, copy this directory to examples/aws/products/<product>/rabbitmq instead."
  }
}
...
variable "shared_broker_name" {
  description = <<-EOT
    ...
    and prd tfvars ship CLUSTER_MULTI_AZ, so a br-sfn release consuming the shared
    broker there sets:
...
variable "subnet_tag_type" {
  description = "Value of the tag:Type used to select the subnets the broker is placed in. \"database\" keeps it aligned with the other three br-sfn datastores; the pre-v2 stack used \"private\"."
  type        = string
  default     = "database"
}
...
# br-sfn workloads run on the EKS nodes, and the broker has to be reachable from
...
variable "console_port" {
  description = "Port of the RabbitMQ management console and management HTTP API. AmazonMQ serves both over HTTPS on 443. NOT emitted into helm_values: the br-sfn chart names no management-port variable — its only AMQP key is correios.secrets.RABBITMQ_URL, a plain amqps:// connection URL. Compare products/midaz/rabbitmq, which emits this as RABBITMQ_PORT_AMQP."
  type        = number
  default     = 443
}

variable "enable_console_ingress" {
  description = "Also open console_port (443) to the resolved ingress sources, so the management console and the management HTTP API stay reachable from inside the VPC. Left true for operator access. Unlike midaz, no br-sfn rail is KNOWN to call the management API — the chart defines no health-check URL variable — so this is the one product where turning it off is a defensible tightening rather than a broken health check."
  type        = bool
  default     = true
}
