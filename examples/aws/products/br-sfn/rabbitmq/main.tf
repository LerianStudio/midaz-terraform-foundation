################################################################################
# products/br-sfn/rabbitmq — the message broker of the br-sfn product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/br-sfn/rabbitmq/terraform.tfstate). Its siblings —
# postgres, valkey and msk — are independent roots with independent state.
#
# SCOPE WARNING. The br-sfn chart names EXACTLY ONE AMQP variable, it belongs to
# a single rail, and it is a SECRET: correios.secrets.RABBITMQ_URL
# (values-template.yaml:80) — a full connection URL carrying the password. That
# is why helm_values here is EMPTY. See outputs.tf.
#
#   mode = "dedicated"  -> creates br-sfn-{env}-rabbitmq[-single|-cluster], its
...
################################################################################
# RabbitMQ — br-sfn-{environment}-rabbitmq[-single|-cluster]
# Secret: br-sfn-{env}-rabbitmq/password   Host: the raw AmazonMQ broker host
################################################################################
...
module "rabbitmq" {
  ...
  # no br-sfn rail is known to call the management HTTP API — this chart
