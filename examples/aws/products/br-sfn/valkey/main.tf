################################################################################
# products/br-sfn/valkey — the cache/lock datastore of the br-sfn product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/br-sfn/valkey/terraform.tfstate). Its siblings —
# postgres, rabbitmq and msk — are independent roots with independent state.
#
# SCOPE WARNING. The br-sfn chart names EXACTLY ONE cache variable, and it
# belongs to a single rail: correios.configmap.CACHE_ADDR
# (values-template.yaml:74, "valkey/redis host:port"). The chart README states
# that the four SPI components "ride one Postgres, one Redis and one RedPanda",
# but no SPI cache variable is named anywhere in the chart — component
# configmaps are emitted verbatim with no allowlist, so the names live in the
# application, not here. See outputs.tf.
#
#   mode = "dedicated"  -> creates br-sfn-{env}-valkey, its security group, its
...
################################################################################
# Valkey — br-sfn-{environment}-valkey
# Secret: br-sfn-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
