...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"br-sfn\" by validation: the derived name (br-sfn-{env}-valkey) and the derived secret path (br-sfn-{env}-valkey/auth-token) ARE the cross-stack discovery contract. A different value here silently produces resources the br-sfn Helm release cannot find."
  type        = string
  default     = "br-sfn"

  validation {
    condition     = var.product == "br-sfn"
    error_message = "The product must be \"br-sfn\". This root stack is the br-sfn Valkey datastore; every name and secret path it produces is derived from it. To provision Valkey for another product, copy this directory to examples/aws/products/<product>/valkey instead."
  }
}
...
# br-sfn workloads run on the EKS nodes, and the cache has to be reachable from
...
variable "transit_encryption_mode" {
  description = "Transit encryption mode: \"preferred\" accepts both TLS and plaintext clients, \"required\" accepts TLS only. KEEP IT PREFERRED. The br-sfn chart names ONE cache variable in the whole repository — correios.configmap.CACHE_ADDR, a host:port string — with no TLS switch and no CA-bundle key, so nothing in the values can tell a rail to speak TLS. \"required\" would lock it out with nothing in the release to explain why."
  type        = string
  default     = "preferred"
...
variable "auth_token_enabled" {
  description = "Make ElastiCache ENFORCE the auth token. The token is generated and stored in Secrets Manager either way; this only decides whether the server requires it. Defaults to false because the br-sfn chart does not yet ship a Valkey TLS/AUTH client configuration — turning it on without that change locks every consumer out."
  type        = bool
  default     = false
}
...
# midaz chart's REDIS_DB key. No such key exists in br-sfn — the chart names one
# cache variable in total (correios.configmap.CACHE_ADDR) — so the variable is
# deliberately absent rather than carried over unused.
################################################################################
