...
variable "product" {
  description = "Product this cluster belongs to. Pinned to \"br-sfn\" by validation: the derived name (br-sfn-{env}-msk) and the derived secret name (AmazonMSK_br-sfn-{env}-msk) ARE the cross-stack discovery contract. A different value here silently produces resources the br-sfn Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "br-sfn"

  validation {
    condition     = var.product == "br-sfn"
    error_message = "The product must be \"br-sfn\". This root stack is the br-sfn MSK cluster; every name and secret path it produces is derived from it. To provision Kafka for another product, copy this directory to examples/aws/products/<product>/msk instead."
  }
}

variable "mode" {
  description = "\"dedicated\" creates an MSK cluster owned by br-sfn (br-sfn-{env}-msk). \"shared\" creates nothing and resolves the cluster owned by products/shared-resources/msk, looked up by name: shared-{env}-msk plus the secret AmazonMSK_shared-{env}-msk. In shared mode this stack resolves no VPC, no subnets and no EKS security group, and plans to zero resources. SHARED IS THE NORMAL CHOICE: three brokers is the real floor for a dedicated cluster (~USD 105/month), and the only argument for paying it here is regulatory isolation of the RSFN rails — a shared Kafka cluster is one topic namespace, one set of ACLs and one retention budget. Note that br-sfn names NO streaming variable in its chart at all, so even the need for a broker is an application fact rather than a chart fact: confirm it with the service owners."
  type        = string
  default     = "dedicated"
...
# "lerian" product label — not "br-sfn", and not "shared" either (that one
# labels the shared DATASTORE tier in products/shared-resources).
################################################################################

variable "vpc_name" {
  description = "tag:Name of the VPC the brokers are placed in. Leave empty (the default) to DERIVE \"lerian-{environment}-vpc\", which is what infra-base/vpc creates and exports as its vpc_name output. Both cross-stack names carry the \"lerian\" FOUNDATION label, not \"br-sfn\" and not \"shared\"."
  type        = string
  default     = ""
}
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"br-sfn\". The lookup matches by tag:Name = \"{cluster}-node\"."
  type        = string
  default     = ""
}
...
variable "enable_sasl_scram" {
  description = "Enable SASL/SCRAM authentication. In dedicated mode the credentials land in Secrets Manager as AmazonMSK_br-sfn-{environment}-msk under a customer managed CMK; the AmazonMSK_ prefix and the CMK are both AWS requirements for a secret associated with a cluster, which is why this is the ONE Lerian datastore whose secret is not named {name}/password. In shared mode this decides whether the shared cluster's secret is resolved at all. Keep it true. MSK SASL/SCRAM is SCRAM-SHA-512 only; whatever configuration key the br-sfn rails use for the mechanism (the chart names none) must carry that value, not the SCRAM-SHA-256 that most examples show."
  type        = bool
  default     = true
}
...
variable "auto_create_topics_enable" {
  description = "Kafka auto.create.topics.enable. The br-sfn chart provisions NO topics — it states that RedPanda topics for spb/spi are an environment concern and that the compose redpanda-topics one-shot is dev-only (README.md:78-79). Kept FALSE anyway, matching every other Lerian cluster: with auto-create on, a typo in a topic name silently produces a live topic with broker defaults and the wrong partition and replication settings. Create the topics deliberately — an rpk job, a Terraform kafka provider or an operator runbook — rather than letting the first publish do it."
  type        = bool
  default     = false
}

variable "default_replication_factor" {
  description = "Kafka default.replication.factor. Null derives min(number_of_broker_nodes, 3). This is the BROKER default and applies to topics created without an explicit factor. It matters more on br-sfn than elsewhere: nothing in this chart creates topics with an explicit factor, so whatever creates them will very likely inherit this value."
  type        = number
  default     = null
}
