...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"plugin-br-pix-switch\" by validation: the derived name (plugin-br-pix-switch-{env}-docdb) and the derived secret path (plugin-br-pix-switch-{env}-docdb/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-pix-switch Helm release cannot find."
  type        = string
  default     = "plugin-br-pix-switch"

  validation {
    condition     = var.product == "plugin-br-pix-switch"
    error_message = "The product must be \"plugin-br-pix-switch\". This root stack is the plugin-br-pix-switch DocumentDB datastore; every name and secret path it produces is derived from it. To provision DocumentDB for another product, copy this directory to examples/aws/products/<product>/documentdb instead."
  }
}
...
# plugin-br-pix-switch workloads run on the EKS nodes, and the cluster has to be reachable from
