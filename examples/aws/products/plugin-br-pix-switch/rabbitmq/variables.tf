...
variable "product" {
  description = "Product this broker belongs to. Pinned to \"plugin-br-pix-switch\" by validation: the derived name (plugin-br-pix-switch-{env}-rabbitmq[-single|-cluster]) and the derived secret path (plugin-br-pix-switch-{env}-rabbitmq/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-pix-switch Helm release cannot find."
  type        = string
  default     = "plugin-br-pix-switch"

  validation {
    condition     = var.product == "plugin-br-pix-switch"
    error_message = "The product must be \"plugin-br-pix-switch\". This root stack is the plugin-br-pix-switch RabbitMQ broker; every name and secret path it produces is derived from it. To provision a broker for another product, copy this directory to examples/aws/products/<product>/rabbitmq instead."
  }
}
...
variable "shared_broker_name" {
  description = <<-EOT
    ...
    and prd tfvars ship CLUSTER_MULTI_AZ, so a plugin-br-pix-switch release consuming the shared
    broker there sets:
...
variable "subnet_tag_type" {
  description = "Value of the tag:Type used to select the subnets the broker is placed in. \"database\" keeps it aligned with the other three plugin-br-pix-switch datastores; the pre-v2 stack used \"private\"."
  type        = string
  default     = "database"
}
...
# plugin-br-pix-switch workloads run on the EKS nodes, and the broker has to be reachable from
...
variable "console_port" {
  description = "Port of the RabbitMQ management console and management HTTP API. AmazonMQ serves both over HTTPS on 443. NOT emitted to the chart: plugin-br-pix-switch names no management-port variable — its only AMQP key is dictHubVsync.secrets.RABBITMQ_URI, a plain amqps:// connection URI. Compare products/midaz/rabbitmq, which emits this as RABBITMQ_PORT_AMQP."
  type        = number
  default     = 443
}

variable "enable_console_ingress" {
  description = "Also open console_port (443) to the resolved ingress sources, so the management console and the management HTTP API stay reachable from inside the VPC. Left true for operator access. Unlike midaz, no plugin-br-pix-switch component calls the management API — the chart names no health-check URL variable — so turning it off is a defensible tightening rather than a broken health check."
  type        = bool
  default     = true
}
