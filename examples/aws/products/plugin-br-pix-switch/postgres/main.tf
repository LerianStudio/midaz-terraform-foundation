################################################################################
# products/plugin-br-pix-switch/postgres — PostgreSQL for the Pix switch
#
# plugin-br-pix-switch is the largest umbrella chart in the fleet: ten
# components (spi, spiSystemplane, adapterBtgMock, dictHub, dictHubVsync,
# dictProxy, dictSystemplane, cobHub, cobProxy, cobSystemplane) plus three
# ingresses. Seven of them talk to Postgres.
#
# THREE DATABASES ON ONE INSTANCE, created by the chart, not by Terraform:
# pix-spi, pix-dict and pix-cob, provisioned by the chart's own bootstrap Job
# (global.externalPostgresDefinitions) using the ADMIN credentials this stack
# creates. See outputs.tf.#
...
# state file (aws/products/plugin-br-pix-switch/postgres/terraform.tfstate). Its
# siblings — documentdb, valkey and rabbitmq — are independent roots with
# independent state, so a RabbitMQ change can never queue behind an RDS apply and
# a corrupt state takes down one datastore instead of four.
#
#   mode = "dedicated"  -> creates plugin-br-pix-switch-{env}-postgres, its security group, its
...
# a naming module seeded with product = "plugin-br-pix-switch" would produce plugin-br-pix-switch-{env}-vpc and
# plugin-br-pix-switch-{env}-eks, which do not exist.
################################################################################
...
################################################################################
# PostgreSQL — plugin-br-pix-switch-{environment}-postgres
# Secret: plugin-br-pix-switch-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
