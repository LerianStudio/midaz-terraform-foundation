...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"plugin-br-pix-switch\" by validation: the derived name (plugin-br-pix-switch-{env}-postgres) and the derived secret path (plugin-br-pix-switch-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-br-pix-switch Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-br-pix-switch"

  validation {
    condition     = var.product == "plugin-br-pix-switch"
    error_message = "The product must be \"plugin-br-pix-switch\". This root stack is the plugin-br-pix-switch PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# plugin-br-pix-switch workloads run on the EKS nodes, and the datastore has to be reachable
...
variable "eks_cluster_name" {
  description = "Name of the EKS cluster whose node security group is resolved. Leave empty (the default) to DERIVE \"lerian-{environment}-eks\" — the cluster belongs to infra-base and carries the \"lerian\" product label, NOT \"plugin-br-pix-switch\". The lookup matches the node security group by tag:Name = \"{cluster}-node\", which is what terraform-aws-modules/eks sets."
  type        = string
  default     = ""
}
...
variable "database_name" {
  description = "Name of the INITIAL database RDS creates. It is NOT one of the three the application uses. plugin-br-pix-switch needs pix-spi, pix-dict and pix-cob, and the CHART creates all three from its own bootstrap Job (global.externalPostgresDefinitions.databases) using the admin credentials this stack provisions. That Job connects to the hardcoded maintenance database \"postgres\", which RDS always provides, so the value here is only the instance's own initial database and nothing in the release reads it."
  type        = string
  default     = "pixswitch"
}

variable "username" {
  description = "Master username of the instance. Keep it \"postgres\": this is the ADMIN account the chart's bootstrap Job authenticates with to run CREATE ROLE and CREATE DATABASE, and global.externalPostgresDefinitions.postgresAdminLogin.username defaults to exactly that (values.yaml:79). The Job then creates the APPLICATION role, default \"pixswitch\" — that one is not created here and its password is not this one. NOTE the URL-safety constraint on the generated password: every component reads Postgres through a postgres:// DSN, so a password containing @ : / ? # % breaks the URL. See README.md."
  type        = string
  default     = "postgres"
}
...
variable "create_read_replica" {
  description = "Create a read replica, plugin-br-pix-switch-{env}-postgres-replica, published as the replica_endpoint output."
  type        = bool
  default     = false
}
