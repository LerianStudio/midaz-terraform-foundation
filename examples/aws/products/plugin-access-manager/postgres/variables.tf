...
variable "product" {
  description = "Product this datastore belongs to. Pinned to \"plugin-access-manager\" by validation: the derived name (plugin-access-manager-{env}-postgres) and the derived secret path (plugin-access-manager-{env}-postgres/password) ARE the cross-stack discovery contract. A different value here silently produces resources the plugin-access-manager Helm release cannot find. A second product gets its own directory under examples/aws/products, not a different value here."
  type        = string
  default     = "plugin-access-manager"

  validation {
    condition     = var.product == "plugin-access-manager"
    error_message = "The product must be \"plugin-access-manager\". This root stack is the plugin-access-manager PostgreSQL datastore; every name and secret path it produces is derived from it. To provision PostgreSQL for another product, copy this directory to examples/aws/products/<product>/postgres instead."
  }
}
...
# reason: the plugin-access-manager workloads run on the EKS nodes, and the instance has to be
...
variable "database_name" {
  description = "Name of the initial database created on the instance. It MUST be \"casdoor\": the auth-backend component hard-codes dbName: casdoor as a literal in its Casdoor Beego config (templates/auth-backend/configmap.yaml:10), where it is not templated from any values key and therefore cannot be overridden from Helm values. The auth component agrees (DB_NAME default \"casdoor\", values.yaml:287), as does the bundled subchart (auth-database.auth.database, values.yaml:417). Changing this value breaks Casdoor, not just the wiring."
  type        = string
  default     = "casdoor"
}
...
variable "create_read_replica" {
  description = "Create a read replica, plugin-access-manager-{env}-postgres-replica, published as the replica_endpoint output."
  type        = bool
  default     = false
}
