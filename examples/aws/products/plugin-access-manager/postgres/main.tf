...
# the Casdoor backing store of the plugin-access-manager product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-access-manager/postgres/terraform.tfstate). Its sibling —
...
#
#   mode = "dedicated"  -> creates plugin-access-manager-{env}-postgres, its security group, its
...
# module seeded with product = "plugin-access-manager" would produce plugin-access-manager-{env}-vpc and
# plugin-access-manager-{env}-eks, which do not exist.
################################################################################
...
################################################################################
# PostgreSQL — plugin-access-manager-{environment}-postgres
# Secret: plugin-access-manager-{env}-postgres/password   Host: the raw RDS endpoint
################################################################################
