...
# the session/token cache of the plugin-access-manager product
#
# ONE ROOT STACK PER SERVICE. This directory owns exactly one datastore and one
# state file (aws/products/plugin-access-manager/valkey/terraform.tfstate). Its sibling —
...
#
#   mode = "dedicated"  -> creates plugin-access-manager-{env}-valkey, its security group, its
...
################################################################################
# Valkey — plugin-access-manager-{environment}-valkey
# Secret: plugin-access-manager-{env}-valkey/auth-token   Host: the raw primary endpoint
################################################################################
