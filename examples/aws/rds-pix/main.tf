...
module "db" {
  ...
  # For gp3 with storage < 400 GiB, AWS does not allow custom IOPS/throughput (baseline is automatic)
  iops               = var.storage_type == "gp3" && var.allocated_storage < 400 ? null : var.iops
  storage_throughput = var.storage_type == "gp3" && var.allocated_storage < 400 ? null : var.storage_throughput
