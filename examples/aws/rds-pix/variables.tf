...
variable "storage_type" {
  description = <<-EOT
    ...
    gp3 is recommended for most workloads with better price-performance.
  EOT
  type        = string
  default     = "gp3"
...
variable "iops" {
  description = <<-EOT
    Provisioned IOPS for the RDS instance.

    For gp3 storage (PostgreSQL):
    - Storage < 400 GiB: MUST be null (baseline 3,000 IOPS is automatic)
    - Storage >= 400 GiB: Optional, baseline is 12,000 IOPS (auto-striping)
    - Maximum: 64,000 IOPS
    - Max ratio: 500 IOPS per GiB of storage

    Set to null to use baseline values (recommended for storage < 400 GiB).
  EOT
  type        = number
  default     = null

  validation {
    condition     = var.iops == null || (var.iops >= 3000 && var.iops <= 64000)
    error_message = "IOPS must be null (for baseline) or between 3,000 and 64,000."
  }
}

variable "storage_throughput" {
  description = <<-EOT
    Storage throughput in MiB/s (only for gp3).

    For gp3 storage (PostgreSQL):
    - Storage < 400 GiB: MUST be null (baseline 125 MiB/s is automatic)
    - Storage >= 400 GiB: Optional, baseline is 500 MiB/s (auto-striping)
    - Maximum: 4,000 MiB/s
    - Max ratio: 0.25 × provisioned IOPS
  ...
  validation {
    condition     = var.storage_throughput == null || (var.storage_throughput >= 125 && var.storage_throughput <= 4000)
    error_message = "Storage throughput must be null (for baseline) or between 125 and 4,000 MiB/s."
  }
}
